...
def test_clear_endpoint(create_server):
    server: Server = create_server()
    db: HTTPRunDB = server.conn

    secrets = _get_model_endpoint_test_secrets()
    ...
    db.clear_endpoint_record(endpoint.metadata.project, endpoint.id)
...
def test_list_endpoints(create_server):
    server: Server = create_server()
    db: HTTPRunDB = server.conn
    ...
    endpoints_out = db.list_model_endpoints(
        "test", token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
...
def test_list_endpoints_filter(create_server):
    server: Server = create_server()
    db: HTTPRunDB = server.conn
    ...
    filter_model = db.list_model_endpoints(
        "test", model="filterme", token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
    assert len(filter_model) == 1

    filter_function = db.list_model_endpoints(
        "test", function="filterme", token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
    assert len(filter_function) == 2

    filter_tag = db.list_model_endpoints(
        "test", tag="filterme", token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
    assert len(filter_tag) == 3

    filter_labels = db.list_model_endpoints(
        "test", labels=["filterme=1"], token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
    assert len(filter_labels) == 4

    filter_labels = db.list_model_endpoints(
        "test", labels=["filterme=1", "filtermey=2"], token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
    assert len(filter_labels) == 4

    filter_labels = db.list_model_endpoints(
        "test", labels=["filtermey=2"], token=secrets[V3IO_ACCESS_KEY]
    ).endpoints
    assert len(filter_labels) == 4
...
def test_get_endpoint_metrics(create_server):
    server: Server = create_server()
    db: HTTPRunDB = server.conn
...
def _get_model_endpoint_test_secrets():
    return {
