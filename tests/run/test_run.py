...
@pytest.mark.parametrize(
    "function_creator,kind,function_name,should_fail",
    [
        # code_to_function tests
        (code_to_function, "serving", "my-function", False),
        (code_to_function, "serving", "my-function-batch", True),
        (code_to_function, "local", "my-local-func", False),
        (code_to_function, "local", "my-local-func-batch", True),
        # new_function tests
        (new_function, "serving", "serving-func", False),
        (new_function, "serving", "serving-func-batch", True),
        (new_function, "local", "local-func", False),
        (new_function, "local", "local-func-batch", True),
    ],
)
def test_batch_suffix_validation(function_creator, kind, function_name, should_fail):
    """
    Test that serving and local functions cannot have names ending with the reserved batch suffix.
    This prevents conflicts when to_job() transforms online functions to batch jobs.
    """
    kwargs = {"name": function_name, "kind": kind}

    # For code_to_function, we need a filename
    if function_creator == code_to_function:
        kwargs["filename"] = function_path

    if should_fail:
        with pytest.raises(mlrun.errors.MLRunValueError) as exc_info:
            function_creator(**kwargs)

        # Verify error message contains the suffix
        error_msg = str(exc_info.value)
        assert (
            "-batch" in error_msg
        ), f"Error message should contain suffix: {error_msg}"
        assert (
        ...
        assert fn.metadata.name == function_name


def test_batch_suffix_validation_job_kind_allowed():
    """
    Test that job functions CAN have names ending with -batch since they don't use to_job().
    """
    # Should not raise an exception
    fn = new_function(name="my-job-batch", kind="job")
    assert fn.metadata.name == "my-job-batch"
