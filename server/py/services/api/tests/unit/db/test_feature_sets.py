...
class TestFeatureSets(TestDatabaseBase):
    @staticmethod
    ...
    def test_create_feature_set(self):
        name = "dummy"
        feature_set = self._create_feature_set(name)

        project = "proj-test"

        feature_set = mlrun.common.schemas.FeatureSet(**feature_set)
        self._db.store_feature_set(
            self._db_session, project, name, feature_set, tag="latest", versioned=True
        )
        self._db.get_feature_set(self._db_session, project, name)
    ...
    def test_handle_feature_set_with_datetime_fields(self):
        # Simulate a situation where a feature-set client-side object is created with datetime fields, and then stored
        # to DB. This may happen in API calls which utilize client-side objects (such as ingest). See ML-3552.
        name = "dummy"
        feature_set = self._create_feature_set(name)
    ...
    def test_update_feature_set_labels(self):
        name = "dummy"
        feature_set = self._create_feature_set(name)
        ...
        feature_set = self._db.get_feature_set(self._db_session, project, name)
        ...
        updated_feature_set = self._db.get_feature_set(self._db_session, project, name)
    ...
    def test_update_feature_set_by_uid(self):
        ...
        updated_feature_set = self._db.get_feature_set(
            self._db_session, project, name, tag=tag, uid=feature_set.metadata.uid
        )
