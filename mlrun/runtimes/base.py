...
class BaseRuntime(ModelObj):
    ...
    def _enrich_run(
        ...
        if runspec.spec.handler_name:
            ...
            def_name += "-" + short_name
        # if name or runspec.metadata.name are set then it means that is user defined name and we want to warn the user
        # that the passed name needs to be set without underscore, if its not user defined but rather enriched from the
        # handler(function) name then we replace the underscore without warning the user.
        # most of the times handlers will have `_` in the handler name (python convention is to separate function words
        # with `_`), therefore we don't want to be noisy when normalizing the run name
        verbose = bool(name or runspec.metadata.name)
        runspec.metadata.name = normalize_name(
            name or runspec.metadata.name or def_name, verbose=verbose
        )
        verify_field_regex(
