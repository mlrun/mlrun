        ...
        if mlrun_constants.MLRunInternalLabels.workflow in run.metadata.labels:
            ...
            runs.extend(
                mlrun.utils.Workflow.get_workflow_steps(
                    run.status.results.get("workflow_id", None), run.metadata.project
                )
            )
...
class CustomNotificationPusher(_NotificationPusherBase):
    ...
    def push(
        ...
        should_push_notification: typing.Optional[bool] = True,
    ):
        def sync_push():
            for notification_type, notification in self._sync_notifications.items():
                if not should_push_notification or self.should_push_notification(
                    notification_type
                ):
