    ...
    def test_exception_handling(self):
        ...
        with pytest.raises(
            ...
            match=r"500 Server Error: Internal Server Error for url: http:\/\/(.*)\/api\/projects\/some-project\/model-"
                  r"endpoints\?start=now-1h&end=now&top-level=False: details: {\'reason\': \"ValueError\(\'Access key m"
                  r"ust be provided in Client\(\) arguments or in the V3IO_ACCESS_KEY environment variable\'\)\"}",
        ):
