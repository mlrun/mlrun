...
def test_enrich_kaniko_env_for_s3_context_keeps_existing_aws_region(monkeypatch):
    # existing AWS_REGION should be used as-is; no S3_FORCE_PATH_STYLE added
    env_vars = [
        client.V1EnvVar(
            name="AWS_REGION",
            value="us-west-1",
    ...
    assert env_map["AWS_REGION"] == "us-west-1"
    assert "S3_FORCE_PATH_STYLE" not in env_map


def test_enrich_kaniko_env_for_s3_context_uses_aws_default_region(monkeypatch):
    env_vars = [
        client.V1EnvVar(
    ...
    env_map = _envs_to_name_value_map(env_vars)
    assert env_map["AWS_REGION"] == "eu-central-1"
    assert "S3_FORCE_PATH_STYLE" not in env_map


def test_enrich_kaniko_env_for_s3_context_uses_process_env_region(monkeypatch):
    env_vars = []
    monkeypatch.setenv(
        "AWS_REGION",
        "ap-south-1",
    ...
    env_map = _envs_to_name_value_map(env_vars)
    assert env_map["AWS_REGION"] == "ap-south-1"
    assert "S3_FORCE_PATH_STYLE" not in env_map


def test_enrich_kaniko_env_for_s3_context_fakes_region_and_sets_path_style(monkeypatch):
    # no region anywhere → fake default-region & add S3_FORCE_PATH_STYLE=true
    env_vars = []
    monkeypatch.delenv(
        "AWS_REGION",
        raising=False,
    ...
    env_map = _envs_to_name_value_map(env_vars)
    assert env_map["AWS_REGION"] == "default-region"
    assert env_map["S3_FORCE_PATH_STYLE"] == "true"


def test_enrich_kaniko_env_for_s3_context_does_not_duplicate_path_style(monkeypatch):
    env_vars = [
        client.V1EnvVar(
    ...
    assert len(_get_envs_by_name(env_vars, "S3_FORCE_PATH_STYLE")) == 1
    assert env_map["S3_FORCE_PATH_STYLE"] == "true"


def test_build_runtime_calls_enrich_kaniko_env_for_s3_source(monkeypatch):
    _patch_k8s_helper(monkeypatch)
    mlrun.mlconf.httpdb.builder.docker_registry = (
        "default.docker.registry/default-repository"
    )
...
def test_build_runtime_does_not_call_enrich_kaniko_env_for_non_s3_source(monkeypatch):
    ...
    function.spec.build.source = "v3io:///projects/some-project/code.tar.gz"
...
def _envs_to_name_value_map(env_vars):
    """Convert list of V1EnvVar into {name: value}."""
    return {env.name: env.value for env in env_vars}


def _get_envs_by_name(env_vars, name):
    """Return list of env vars that match a given name."""
    return [env for env in env_vars if env.name == name]
