...
async def store_datastore_profile(
    ...
    await run_in_threadpool(
        server.api.utils.singletons.db.get_db().store_datastore_profile,
        db_session,
        info,
    )

    return await run_in_threadpool(
        server.api.utils.singletons.db.get_db().get_datastore_profile,
        db_session,
        info.name,
        project_name,
    )
...
async def list_datastore_profiles(
    ...
    profiles = await run_in_threadpool(
        server.api.utils.singletons.db.get_db().list_datastore_profiles,
        db_session,
        project_name,
    )
    if len(profiles) == 0:
        return profiles
    filtered_data = await server.api.utils.auth.verifier.AuthVerifier().filter_project_resources_by_permissions(
        mlrun.common.schemas.AuthorizationResourceTypes.datastore_profile,
        profiles,
...
async def delete_datastore_profile(
    ...
    return await run_in_threadpool(
        server.api.utils.singletons.db.get_db().delete_datastore_profile,
        db_session,
        profile,
        project_name,
    )
