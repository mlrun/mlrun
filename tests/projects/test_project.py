...
def test_import_artifact_retain_producer(rundb_mock):
    project_1 = mlrun.new_project(
        name="project-1", context=f"{str(assets_path())}/project_1", save=False
    )
    project_2 = mlrun.new_project(
        name="project-2", context=f"{str(assets_path())}/project_2", save=False
    )
    ...
    # export the artifact
    artifact_path = f"{str(assets_path())}/my-artifact.yaml"
