...
This section illustrates passing data and logging it without using the decorator. <br>
...
### Demo functions
```
# mlrun: start-codefrom typing import Tuple
...
Since there is a type hint, MLRun parses it automatically to the type hinted. If the type hinted is not known to MLRun, mlrun.DataItem is placed instead.
```
...
log_with_returns_run = notebook_functions.run(
    ...
    inputs={"df": DATA_PATH},
    returns=["reutrns_data", "returns_result"],
    local=True,
)
```
