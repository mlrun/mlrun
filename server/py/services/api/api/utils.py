...
from server.py.services.api.utils.singletons.logs_dir import get_logs_dir
...
def apply_enrichment_and_validation_on_task(task):
    ...
    mask_notification_params_on_task(
        task, server.py.services.api.constants.MaskOperations.CONCEAL
    )


def mask_notification_params_on_task(
    task: dict,
    action: server.py.services.api.constants.MaskOperations,
):
    """
...
def _notification_params_mask_op(
    ...
    return {
        server.py.services.api.constants.MaskOperations.CONCEAL: _conceal_notification_params_with_secret,
        server.py.services.api.constants.MaskOperations.REDACT: _redact_notification_params,
    }[action]
    ...
    if (
        ...
        secret_key = (
            server.py.services.api.crud.Secrets().generate_client_project_secret_key(
                server.py.services.api.crud.SecretsClientType.notifications,
...
def unmask_notification_params_secret_on_task(
    db: server.py.services.api.db.base.DBInterface,
    db_session: Session,
...
def process_function_service_account(function):
    ...
    if not server.py.services.api.utils.singletons.k8s.get_k8s_helper(
        silent=True
...
def resolve_project_default_service_account(project_name: str):
    allowed_service_accounts = server.py.services.api.crud.secrets.Secrets().get_project_secret(
        project_name,
        mlrun.common.schemas.SecretProviderName.kubernetes,
    ...
    default_service_account = server.py.services.api.crud.secrets.Secrets().get_project_secret(
        project_name,
        mlrun.common.schemas.SecretProviderName.kubernetes,
        server.py.services.api.crud.secrets.Secrets().generate_client_project_secret_key(
            server.py.services.api.crud.secrets.SecretsClientType.service_accounts,
