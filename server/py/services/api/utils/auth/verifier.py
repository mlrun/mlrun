...
import server.py.services.api.utils.auth.providers.nop
...
class AuthVerifier(metaclass=mlrun.utils.singleton.Singleton):
    ...
    def __init__(self) -> None:
        ...
        if mlrun.mlconf.httpdb.authorization.mode == "none":
            self._auth_provider = (
                server.py.services.api.utils.auth.providers.nop.Provider()
            )
    ...
    async def authenticate_request(
        ...
        if self._basic_auth_configured():
            ...
            iguazio_client = server.py.services.api.utils.clients.iguazio.AsyncClient()
    ...
    def get_or_create_access_key(self, session: str, planes: list[str] = None) -> str:
        ...
        return server.py.services.api.utils.clients.iguazio.Client().get_or_create_access_key(
            session, planes
        )
