...
async def get_model_monitoring_url(
    ...
    stream pod). If the stream pod is not in a ready state, the URL is still returned
    (with a server-side warning) — it may not be reachable until the pod becomes ready.
