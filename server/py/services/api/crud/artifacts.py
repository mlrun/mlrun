...
class Artifacts(
    ...
    def store_artifact(
        ...
        return server.py.services.api.utils.singletons.db.get_db().store_artifact(
            db_session,
            key,
            artifact,
            object_uid,
    ...
    def create_artifact(
        ...
        return server.py.services.api.utils.singletons.db.get_db().create_artifact(
            db_session,
            project,
            artifact,
            key,
    ...
    def get_artifact(
        ...
        project = project or mlrun.mlconf.default_project
        artifact = server.py.services.api.utils.singletons.db.get_db().read_artifact(
            db_session,
            key,
            tag,
            iter,
    ...
    def list_artifacts(
        ...
        if labels is None:
            labels = []
        artifacts = server.py.services.api.utils.singletons.db.get_db().list_artifacts(
            db_session,
            name,
            project,
            tag,
    ...
    def list_artifact_tags(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().list_artifact_tags(
    ...
    def delete_artifact(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().del_artifact(
    ...
    def delete_artifacts(
        ...
        project = project or mlrun.mlconf.default_project
        server.py.services.api.utils.singletons.db.get_db().del_artifacts(
    ...
    def _resolve_artifact_size(artifact, auth_info):
        if "spec" in artifact and "size" not in artifact["spec"]:
            if "target_path" in artifact["spec"]:
                ...
                try:
                    file_stat = server.py.services.api.crud.Files().get_filestat(
                        auth_info, path=path
                    )
