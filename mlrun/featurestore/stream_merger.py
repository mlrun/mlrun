from storey import (JoinWithV3IOTable, V3ioDriver, QueryAggregationByKey, Cache,
                    Map, Source, FieldAggregator, build_flow, Reduce)
...
def steps_from_featureset(featureset: FeatureSet):
    ...
    if aggregations:
        ...
        for name, aggregate in aggregations.items():
            ...
            aggregator = FieldAggregator(name, aggregate.column, aggregate.operations,
    ...
    steps.append(JoinWithV3IOTable(V3ioDriver(), lambda x: x[key_column],
                                   join_event, target.path, attributes=column_list))
    return steps
...
class QueryInput:
    ...
    def get_steps(self):
        ...
        if isinstance(self.columns[0], str):
            steps.append(JoinWithV3IOTable(V3ioDriver(), lambda x: x[self.key_column], join_event, self.kv_table))
        else:
...
def test_build_query():
    ...
    params = [QueryInput('bigdata/aggr_test/ajCRMtLOms',
                         [FieldAggregator('number_of_stuff', 'col1', ['sum', 'count'], SlidingWindows(['1h', '24h'], '10m'))],
                         'name', None),
              QueryInput('bigdata/users_metadata', ['city', 'age'], 'name', {'city': 'hometown'})]
