...
from server.py.services.api.crud.model_monitoring.helpers import Seconds
from server.py.services.api.crud.secrets import Secrets, SecretsClientType
from server.py.services.api.utils.builder import build_runtime
...
async def list_functions(
    ...
    functions = await server.py.services.api.utils.auth.verifier.AuthVerifier().filter_project_resources_by_permissions(
        mlrun.common.schemas.AuthorizationResourceTypes.function,
        functions,
        lambda function: (
...
def _handle_job_deploy_status(
    ...
    # read from log file
    log_file = server.py.services.api.api.utils.log_path(
    ...
    build_pod_state = server.py.services.api.utils.singletons.k8s.get_k8s_helper(
        silent=False
    ).get_pod_status(pod)
    logger.debug(
        "Resolved pod status",
    ...
    if (
        ...
        try:
            resp = server.py.services.api.utils.singletons.k8s.get_k8s_helper(
                silent=False
            ).logs(pod)
...
def _handle_nuclio_deploy_status(
    ...
    (
        ...
        text,
        status,
    ) = server.py.services.api.crud.runtimes.nuclio.function.get_nuclio_deploy_status(
        name,
        project,
        tag,
...
def _build_function(
    ...
    try:
        ...
        launcher = server.py.services.api.launcher.ServerSideLauncher(
            auth_info=auth_info
        )
...
def _parse_start_function_body(db_session, data):
    ...
    runtime = server.py.services.api.crud.Functions().get_function(
...
async def _get_function_status(data, auth_info: mlrun.common.schemas.AuthInfo):
    ...
    try:
        status = server.py.services.api.crud.Functions().get_function_status(
            kind,
            selector,
        )
