...
import server.py.services.api.utils.clients.nuclio
import server.py.services.api.utils.events.events_factory as events_factory
import server.py.services.api.utils.projects.remotes.follower as project_follower
...
class Projects(
    ...
    def create_project(
        ...
        server.py.services.api.utils.singletons.db.get_db().create_project(
            session, project
        )
    ...
    def store_project(
        ...
        server.py.services.api.utils.singletons.db.get_db().store_project(
    ...
    def patch_project(
        ...
        server.py.services.api.utils.singletons.db.get_db().patch_project(
    ...
    def delete_project(
        ...
        if (
            ...
            if not server.py.services.api.utils.singletons.db.get_db().is_project_exists(
                session, name
            ):
                return
        ...
        server.py.services.api.utils.singletons.db.get_db().delete_project(
    ...
    def verify_project_is_empty(
        ...
        server.py.services.api.utils.singletons.db.get_db().verify_project_has_no_related_resources(
            session, name
        )
    ...
    def _verify_project_has_no_external_resources(
        ...
        server.py.services.api.crud.ModelEndpoints().verify_project_has_no_model_endpoints(
            project
        )
        ...
        if mlrun.config.config.nuclio_dashboard_url:
            nuclio_client = server.py.services.api.utils.clients.nuclio.Client()
    ...
    def delete_project_resources(
        ...
        server.py.services.api.utils.singletons.scheduler.get_scheduler().delete_schedules(
            session, name
        )
        ...
        if mlrun.mlconf.resolve_kfp_url():
            ...
            server.py.services.api.crud.pipelines.Pipelines().delete_pipelines_runs(
        ...
        if (
            ...
            server.py.services.api.crud.Logs().delete_project_logs_legacy(name)

        # delete db resources
        server.py.services.api.utils.singletons.db.get_db().delete_project_related_resources(
            session, name
        )
        ...
        server.py.services.api.crud.ModelEndpoints().delete_model_endpoints_resources(
            name
        )
    ...
    def get_project(
        ...
        return server.py.services.api.utils.singletons.db.get_db().get_project(
            session, name
        )
    ...
    def list_projects(
        ...
        return server.py.services.api.utils.singletons.db.get_db().list_projects(
    ...
    async def _get_project_resources_counters(
        ...
        if (
            ...
            results = await asyncio.gather(
                server.py.services.api.utils.singletons.db.get_db().get_project_resources_counters(),
    ...
    def _list_pipelines(
        ...
        return server.py.services.api.crud.Pipelines().list_pipelines(
            session, "*", format_=format_
        )
    ...
    def _wait_for_nuclio_project_deletion(
        ...
        def _verify_no_project_function_pods():
            project_function_pods = server.py.services.api.utils.singletons.k8s.get_k8s_helper().list_pods(
