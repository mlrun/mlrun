...
class TestAggregationQueries:
    ...
    def test_get_avg_latency_raw_query(self, query_handler_with_aggregates):
        ...
        result_multi = query_handler_with_aggregates.get_avg_latency(
    ...
    def test_get_avg_latency_with_pre_aggregate_interval(
        self, query_handler_with_aggregates, admin_connection
    ):
    ...
    def test_get_drift_status_raw_query(self, query_handler_with_aggregates):
    ...
    def test_get_drift_status_with_pre_aggregate_interval(
        ...
        assert (
            len(endpoint_values.intersection({"test_endpoint", "other_endpoint"})) >= 1
        )
...
class TestPreAggregateExceptionHandling:
    ...
    def test_get_avg_latency_pre_aggregate_exception_fallback(
        self, query_handler_with_aggregates
    ):
...
class TestLatestMetricsCalculation:
    ...
    def test_calculate_latest_metrics_empty_application_list(self, query_handler):
    ...
    def test_calculate_latest_metrics_with_drift_data_only(self, query_handler):
        ...
        assert isinstance(result, list)
        assert len(result) == 1  # Should have 1 result based on actual behavior

        # Verify the results contain ApplicationResultRecord objects
        for item in result:
            # Should be either ApplicationResultRecord or ApplicationMetricRecord
            assert item.type in ["result", "metric"], f"Invalid type: {item.type}"
            assert item.value is not None, "Expected item to have value"
            assert item.time is not None, "Expected item to have time"

            # For ApplicationResultRecord, verify additional attributes
            if item.type == "result":
                assert item.kind is not None, "Expected result item to have kind"
...
class TestEndpointCounting:
    ...
    def test_count_processed_model_endpoints_with_data(self, query_handler):
        ...
        result_other = query_handler.count_processed_model_endpoints(
            start=datetime(2024, 1, 15),
            end=datetime(2024, 1, 16),
        ...
        assert "other_app" in result_other
    ...
    def test_count_processed_model_endpoints_time_filtering(self, query_handler):
        ...
        assert isinstance(result, dict)
        # Should only count endpoints within the time range, but we need application linkage
