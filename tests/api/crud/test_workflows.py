...
class TestWorkflows(tests.api.conftest.MockedK8sHelper):
    ...
    @pytest.mark.parametrize(
        ...
        [
            (server.api.crud.WorkflowRunners, "./project-name", False),
            (server.api.crud.WorkflowRunners, "", True),
            (server.api.crud.LoadRunner, "s3://project-name", True),
            (server.api.crud.LoadRunner, "", True),
        ],
    )
