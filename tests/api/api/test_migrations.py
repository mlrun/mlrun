...
def test_start_migration_already_in_progress(
    ...
    mlrun.mlconf.httpdb.state = mlrun.api.schemas.APIStates.migration_in_progress
    response = client.post("/api/migrations/start")
    assert response.status_code == http.HTTPStatus.CONFLICT.value
