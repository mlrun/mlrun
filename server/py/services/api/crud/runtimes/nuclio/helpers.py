...
def compile_nuclio_archive_config(
    ...
    if (
        project
        and server.py.services.api.utils.singletons.k8s.get_k8s_helper().is_running_inside_kubernetes_cluster()
    ):
        secrets = server.py.services.api.utils.singletons.k8s.get_k8s_helper().get_project_secret_data(
            project
        )
