...
|--------|----------------------------------------------------------------|-----------------------------------------|-----------|
|ML-1373|When attempting to ingest data with RemoteSpark using an incorrect service name, the pods get stuck during ContainerCreating. | NA | v0.9.0 | 
...
|ML-4613|UI: The Batch Inference Parameters has incorrect hint on map type. The correct hint is "The 'dict' values should be in JSON key:value format, e.g. {"hello":"world"}" | NA | v1.5.0 |
...
|ML-4942|Dask dashboard does not work on managed clouds.|Dask is externally available thru NodePort, meaning ports needs to be manually open by the infrastructure provider. If running MLRun locally or CE, make sure to port-forward the port Dask Dashboard uses to ensure it is available externally to the Kubernetes cluster. | v1.5.0 |
|ML-4992|Source target dir ownership is set to root. |Set target dir, and change workdir back: 
...
| ID     |Description                                                                                                                                                                          |Workaround                                                                                                                                                                                                                                                                                                                      |Opened in |
|---------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------|     
|ML-1278|A user's permission to store data on the underlying storage depends on its V3IO permissions, not on project membership.                                                               |NA                                                               | v0.8.0
...
|ML-4386|Local notifications aren’t persisted in mlrun API| NA | v1.5.0|
|ML-4767|Use PyTorch versions up to and including than 2.0.1, but not higher.       | NA |                 v1.5.0|   
|ML-4960|The Memberships tab in Projects > Settings is part of MLRun only when integrated with the platform.            | NA |                 v1.5.0| 
