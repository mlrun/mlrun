...
import server.py.services.api.db.init_db
...
def test_add_data_version_empty_db():
    ...
    original_latest_data_version = (
        server.py.services.api.initial_data.latest_data_version
    )
    server.py.services.api.initial_data.latest_data_version = "3"
    assert db.get_current_data_version(db_session, raise_on_not_found=False) is None
    server.py.services.api.initial_data._add_initial_data(db_session)
    assert (
...
def test_perform_data_migrations_from_first_version():
    ...
    original_perform_version_2_data_migrations = (
        server.py.services.api.initial_data._perform_version_2_data_migrations
    )
    server.py.services.api.initial_data._perform_version_2_data_migrations = (
        unittest.mock.Mock()
    )
    original_perform_version_3_data_migrations = (
        server.py.services.api.initial_data._perform_version_3_data_migrations
    )
    server.py.services.api.initial_data._perform_version_3_data_migrations = (
        unittest.mock.Mock()
    )
    original_perform_version_4_data_migrations = (
        server.py.services.api.initial_data._perform_version_4_data_migrations
    )
    server.py.services.api.initial_data._perform_version_4_data_migrations = (
        unittest.mock.Mock()
    )
    original_perform_version_5_data_migrations = (
        server.py.services.api.initial_data._perform_version_5_data_migrations
    )
    server.py.services.api.initial_data._perform_version_5_data_migrations = (
        unittest.mock.Mock()
    )
    ...
    # perform migrations
    server.py.services.api.initial_data._perform_data_migrations(db_session)

    # calling again should not trigger migrations again, since we're already at the latest version
    server.py.services.api.initial_data._perform_data_migrations(db_session)

    server.py.services.api.initial_data._perform_version_2_data_migrations.assert_called_once()
    server.py.services.api.initial_data._perform_version_3_data_migrations.assert_called_once()
    server.py.services.api.initial_data._perform_version_4_data_migrations.assert_called_once()
    server.py.services.api.initial_data._perform_version_5_data_migrations.assert_called_once()
...
def test_resolve_current_data_version_version_exists():
    ...
    db.create_data_version(db_session, "1")
    assert (
        server.py.services.api.initial_data._resolve_current_data_version(
            db, db_session
        )
        == 1
    )
...
def test_add_default_hub_source_if_needed():
    ...
    server.py.services.api.initial_data._add_default_hub_source_if_needed(
        db, db_session
    )
    hub_source = db.get_hub_source(
        db_session,
    ...
    with unittest.mock.patch(
        "server.py.services.api.initial_data._update_default_hub_source"
...
def _initialize_db_without_migrations() -> (
    tuple[server.py.services.api.db.sqldb.db.SQLDB, sqlalchemy.orm.Session]
):
    ...
    db = server.py.services.api.db.sqldb.db.SQLDB(dsn)
    db.initialize(db_session)
    server.py.services.api.db.init_db.init_db()
