...
class TestInjectMonitoringEnvVarsApplicationSidecar:
    """
    Coverage for ApplicationRuntime: monitoring env vars must be propagated to
    the user's application sidecar (``spec.config["spec.sidecars"][*].env``),
    not only to the top-level ``spec.env`` which on the rendered Nuclio CRD
    ends up on the reverse-proxy container.

    These tests are expected to FAIL until the injector is made sidecar-aware
    (gap discovered during the Streamline demo on 2026-06-02 — the user's
    Flask app in the sidecar never sees MODEL_MONITORING_URL /
    MODEL_ENDPOINT_UID / MODEL_ENDPOINT_NAME / MODEL_ENDPOINTS_MAP).
    """

    @staticmethod
    def _app_function_dict(sidecar_env: list[dict] | None = None) -> dict:
        """ApplicationRuntime-shaped function dict: sidecar lives under
        ``spec.config["spec.sidecars"]`` (Nuclio dot-keyed config map)."""
        return {
            "kind": "application",
            "spec": {
                "env": [],
    ...
    def test_inject_propagates_to_sidecar_env(self):
        fn = self._app_function_dict()
        mm_dep.MonitoringDeployment.inject_monitoring_env_vars(
            fn,
            {
                ...
                mm_constants.NuclioMonitoringEnvVars.MODEL_ENDPOINT_UID: "uid-1",
                mm_constants.NuclioMonitoringEnvVars.MODEL_ENDPOINT_NAME: "ep1",
            },
        )
        sidecar_env = {
            e["name"]: e["value"]
            for e in fn["spec"]["config"]["spec.sidecars"][0]["env"]
        }
        assert (
            sidecar_env.get(mm_constants.NuclioMonitoringEnvVars.MODEL_MONITORING_URL)
            == "http://stream:8080"
        ), "MODEL_MONITORING_URL must reach the user's sidecar, not just the reverse-proxy"
        assert (
            sidecar_env.get(mm_constants.NuclioMonitoringEnvVars.MODEL_ENDPOINT_UID)
            == "uid-1"
        )
        assert (
            sidecar_env.get(mm_constants.NuclioMonitoringEnvVars.MODEL_ENDPOINT_NAME)
            == "ep1"
        )

    def test_inject_preserves_existing_sidecar_env(self):
        fn = self._app_function_dict(
            sidecar_env=[{"name": "USER_VAR", "value": "keep-me"}]
        )
        mm_dep.MonitoringDeployment.inject_monitoring_env_vars(
            fn,
            {mm_constants.NuclioMonitoringEnvVars.MODEL_ENDPOINT_UID: "uid-2"},
        )
        sidecar_env = {
            e["name"]: e["value"]
            for e in fn["spec"]["config"]["spec.sidecars"][0]["env"]
        }
        assert sidecar_env.get("USER_VAR") == "keep-me", (
            "User-defined sidecar env must be preserved"
        )
        assert (
            sidecar_env.get(mm_constants.NuclioMonitoringEnvVars.MODEL_ENDPOINT_UID)
            == "uid-2"
    ...
    def test_build_and_inject_propagates_to_sidecar_for_application_runtime(self):
        """End-to-end through the same call path ApplicationRuntime hits on
        deploy(track_models=True): server invokes
        _build_and_inject_monitoring_env_vars on the function dict."""
        from mlrun.common.schemas.model_monitoring.constants import (
            ModelEndpointCreationStrategy,
        )
        ...
        ep = mlrun.common.schemas.ModelEndpoint(
            metadata=mlrun.common.schemas.model_monitoring.ModelEndpointMetadata(
                project="proj", name="ep1", uid="uid-111"
            ),
            spec=mlrun.common.schemas.model_monitoring.ModelEndpointSpec(),
            status=mlrun.common.schemas.model_monitoring.ModelEndpointStatus(),
        )
        dep = mm_dep.MonitoringDeployment(project="proj")
        fn = dep._build_and_inject_monitoring_env_vars(
            function=self._app_function_dict(),
            model_endpoints_instructions=[(ep, ModelEndpointCreationStrategy.INPLACE)],
            stream_url="http://stream:8080",
        )
        sidecar_env = {
