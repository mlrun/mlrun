...
def make_kaniko_pod(
    ...
    kpod = server.py.services.api.utils.singletons.k8s.BasePod(
        name or "mlrun-build",
...
def build_image(
    ...
    if (
        ...
        from server.py.services.api.api.utils import ensure_function_security_context
    ...
    k8s = server.py.services.api.utils.singletons.k8s.get_k8s_helper(silent=False)
...
def build_runtime(
    ...
    if is_mlrun_image and build.requirements:
        (
            image_tag,
            has_py_package,
        ) = server.py.services.api.utils.helpers.extract_image_tag(enriched_base_image)
