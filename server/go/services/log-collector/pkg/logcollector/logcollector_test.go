...
import (
	...
	"github.com/mlrun/log-collector/pkg/logcollector/test/nop"
...
func (suite *LogCollectorTestSuite) TestStartLogBestEffort() {
	...
	request := &log_collector_v1.StartLogRequest{
		RunUID:      "some-run-id",
...
func (suite *LogCollectorTestSuite) TestGetLogsWithSize() {
	...
	for _, testCase := range []struct {
		...
		suite.Run(testCase.name, func() {
			...
			err := suite.logCollectorServer.GetLogs(&log_collector_v1.GetLogsRequest{
				RunUID:      runUID,
...
func (suite *LogCollectorTestSuite) TestGetLogSize() {
	runUID := uuid.New().String()
	request := &log_collector_v1.GetLogSizeRequest{
		RunUID:      runUID,
		ProjectName: suite.projectName,
	}
...
func (suite *LogCollectorTestSuite) TestStopLog() {
	...
	for project, runs := range projectToRuns {
		request := &log_collector_v1.StopLogsRequest{
			Project: project,
			RunUIDs: runs,
		}
