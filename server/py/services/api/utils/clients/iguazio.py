...
class Client(
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._wait_for_job_completion_retry_interval = mlrun.utils.create_step_backoff([[1, 10], [5, None]])
    ...
    def verify_request_session(self, request: fastapi.Request) -> mlrun.common.schemas.AuthInfo:
        """
        ...
        return self._generate_auth_info_from_session_verification_response(response.headers, response.json())
    ...
    def delete_project(
        ...
        with self._job_cache.get_delete_lock(name):
            ...
            if not job_id:
                job_id = self._delete_project_in_iguazio(session, name, deletion_strategy)
                if not job_id:
    ...
    def list_projects(
        ...
        project_names, latest_updated_at = self._list_project_names(session, updated_after, page_size)
    ...
    def get_project_owner(
        ...
        response = self._get_project_from_iguazio_without_parsing(session, name, enrich_owner_access_key=True)
        iguazio_project = response.json()["data"]
        owner_username = iguazio_project.get("attributes", {}).get("owner_username", None)
        owner_access_key = iguazio_project.get("attributes", {}).get("owner_access_key", None)
        if not owner_username:
            raise mlrun.errors.MLRunInternalServerError(
                f"Unable to enrich project owner for project {name}," f" because project has no owner configured"
            )
    ...
    @services.api.utils.helpers.lru_cache_with_ttl(maxsize=1, ttl_seconds=60 * 2)
    ...
    def _list_project_names(
        ...
        if page_size is None:
            page_size = mlrun.mlconf.httpdb.projects.iguazio_list_projects_default_page_size
        ...
        response_body = response.json()
        project_names = [iguazio_project["attributes"]["name"] for iguazio_project in response_body["data"]]
        latest_updated_at = self._find_latest_updated_at(response_body)
        return project_names, latest_updated_at

    def _list_projects_data(self, session: str, project_names: list[str]) -> list[mlrun.common.schemas.Project]:
        return [self._get_project_from_iguazio(session, project_name) for project_name in project_names]
    ...
    def _find_latest_updated_at(self, response_body: dict) -> typing.Optional[datetime.datetime]:
        ...
        for iguazio_project in response_body["data"]:
            updated_at = datetime.datetime.fromisoformat(iguazio_project["attributes"]["updated_at"])
    ...
    def _delete_project_in_iguazio(
        ...
        body = self._transform_mlrun_project_to_iguazio_project(
            mlrun.common.schemas.Project(metadata=mlrun.common.schemas.ProjectMetadata(name=name))
        )
        headers = {
        ...
        try:
            ...
            raise mlrun.errors.MLRunNotFoundError("Project not found in Iguazio") from exc
    ...
    def _get_project_from_iguazio_without_parsing(self, session: str, name: str, enrich_owner_access_key: bool = False):
    ...
    def _wait_for_job_completion(
        ...
        if job_state != JobStates.completed:
            ...
            try:
                ...
                final_error_message = self._resolve_final_error_message(error_message, result_message)
    ...
    def _send_request_to_api(
        ...
        **kwargs,
        ...
        response = http_session.request(method, url, verify=mlrun.mlconf.httpdb.http.verify, **kwargs)
        if not response.ok:
            try:
                response_body = response.json()
            except Exception:
    ...
    def _resolve_params_from_response_body(
        ...
        context_auth = get_in(response_body, "data.attributes.context.authentication", {})
    ...
    def _transform_mlrun_project_to_iguazio_project(
        ...
        body = {
            "data": {
                ...
                "attributes": {
                    ...
                    "mlrun_project": Client._transform_mlrun_project_to_iguazio_mlrun_project_attribute(project),
                },
            }
        }
        if project.metadata.created:
            body["data"]["attributes"]["created_at"] = project.metadata.created.isoformat()
        if project.metadata.labels is not None:
            body["data"]["attributes"]["labels"] = Client._transform_mlrun_labels_to_iguazio_labels(
                project.metadata.labels
            )
        if project.metadata.annotations is not None:
            body["data"]["attributes"]["annotations"] = Client._transform_mlrun_labels_to_iguazio_labels(
        ...
        if project.spec.default_function_node_selector is not None:
            ...
            if mlrun.utils.helpers.validate_component_version_compatibility("iguazio", "3.5.5"):
                body["data"]["attributes"]["default_function_node_selector"] = (
                    Client._transform_mlrun_labels_to_iguazio_labels(project.spec.default_function_node_selector)
                )
            else:
                logger.debug(
    ...
    def _transform_iguazio_project_to_mlrun_project(
        ...
        mlrun_project_without_common_fields = json.loads(iguazio_project["attributes"].get("mlrun_project", "{}"))
        # name is mandatory in the mlrun schema, without adding it the schema initialization will fail
        mlrun_project_without_common_fields.setdefault("metadata", {})["name"] = iguazio_project["attributes"]["name"]
        mlrun_project = mlrun.common.schemas.Project(**mlrun_project_without_common_fields)
        mlrun_project.metadata.created = datetime.datetime.fromisoformat(iguazio_project["attributes"]["created_at"])
        ...
        if iguazio_project["attributes"].get("description"):
            mlrun_project.spec.description = iguazio_project["attributes"]["description"]
        ...
        if iguazio_project["attributes"].get("default_function_node_selector"):
            mlrun_project.spec.default_function_node_selector = Client._transform_iguazio_labels_to_mlrun_labels(
    ...
    def _prepare_request_kwargs(self, session, path, *, kwargs):
        ...
        if session_cookie:
            ...
            if "session" in cookies and cookies["session"] != session_cookie:
                raise mlrun.errors.MLRunInvalidArgumentError("Session cookie already set")
            cookies["session"] = session_cookie
            kwargs["cookies"] = cookies
        if kwargs.get("timeout") is None:
            kwargs["timeout"] = 20
        if "projects" in path:
            if mlrun.common.schemas.HeaderNames.projects_role not in kwargs.get("headers", {}):
                kwargs.setdefault("headers", {})[mlrun.common.schemas.HeaderNames.projects_role] = "mlrun"
    ...
    def _is_job_terminated(self, session: str, job_id: str) -> bool:
        ...
        try:
            ...
            return response["data"]["attributes"]["state"] in JobStates.terminal_states()
        except requests.HTTPError as exc:
            if exc.response.status_code == http.HTTPStatus.NOT_FOUND.value:
                raise mlrun.errors.MLRunNotFoundError(f"Job {job_id} not found in Iguazio")
            raise
...
class AsyncClient(Client):
    ...
    async def verify_request_session(self, request: fastapi.Request) -> mlrun.common.schemas.AuthInfo:
        """
        Proxy the request to one of the session verification endpoints (which will verify the session of the request)
        """
        headers = {
        ...
        async with self._send_request_to_api_async(
            ...
            return self._generate_auth_info_from_session_verification_response(response.headers, await response.json())
    ...
    async def _send_request_to_api_async(
        ...
        **kwargs,
        ...
        response = None
        try:
            response = await self._async_session.request(method, url, verify_ssl=False, **kwargs)
            if not response.ok:
                try:
    ...
    async def _ensure_async_session(self, blacklisted_methods=None):
        if not self._async_session or self._async_session.methods_blacklist_update_required(blacklisted_methods):
