...
- {py:meth}`~mlrun.projects.MlrunProject.set_model_monitoring_credentials` &mdash; Set the credentials that are used by the project's model monitoring infrastructure functions. You must set the credentials before deploying any model monitoring or serving function.
...
- {py:meth}`~mlrun.projects.MlrunProject.delete_model_monitoring_function` &mdash; Delete the specified model-monitoring-app function/s
