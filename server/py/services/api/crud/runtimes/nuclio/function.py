...
def deploy_nuclio_function(
    ...
    server.py.services.api.crud.runtimes.nuclio.helpers.enrich_function_with_ingress(
        function_config,
...
def _resolve_env_vars(function):
    ...
    if server.py.services.api.utils.singletons.k8s.get_k8s_helper(
        silent=True
    ...
    if (
        server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
            "1.6.0", "1.8.0"
        )
...
def _resolve_and_set_nuclio_runtime(
    ...
    nuclio_runtime = (
        ...
        or server.py.services.api.crud.runtimes.nuclio.helpers.resolve_nuclio_runtime_python_image(
    ...
    if server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
        "0.0.0", "1.6.0"
...
def _set_build_params(function, nuclio_spec, builder_env, project, auth_info=None):
    ...
    if function.spec.build.source:
        server.py.services.api.crud.runtimes.nuclio.helpers.compile_nuclio_archive_config(
    ...
    image_pull_secret = server.py.services.api.crud.runtimes.nuclio.helpers.resolve_function_image_pull_secret(
        function
    )
    ...
    if function.spec.build.extra_args:
        nuclio_spec.set_config(
            "spec.build.flags",
            server.py.services.api.crud.runtimes.nuclio.helpers.parse_extra_args_to_nuclio_build_flags(
...
def _resolve_and_set_base_image(
    ...
    if base_image:
        base_image = server.py.services.api.utils.builder.resolve_image_target(
            base_image
        )
        mlrun.utils.update_in(
            config,
...
def _add_secrets_config_to_function_spec(
    ...
    handler = server.py.services.api.runtime_handlers.BaseRuntimeHandler
    if function.kind in [
