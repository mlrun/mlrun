        ...
        if not self._model_endpoint:
            if self.endpoint_name is None or self.endpoint_id is None:
                raise mlrun.errors.MLRunValueError(
                    "You have NOT provided the model endpoint's name and ID, but you have tried to access "
                    "`monitoring_context.model_endpoint` directly or indirectly in your application. "
                    "You can either provide them, or adapt the application's logic to not access the "
                    "model endpoint."
                )
