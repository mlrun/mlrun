...
import server.api.utils.auth.providers.nop
...
class AuthVerifier(metaclass=mlrun.utils.singleton.Singleton):
    ...
    def __init__(self) -> None:
        ...
        if mlrun.mlconf.httpdb.authorization.mode == "none":
            self._auth_provider = server.api.utils.auth.providers.nop.Provider()
        elif mlrun.mlconf.httpdb.authorization.mode == "opa":
            self._auth_provider = server.api.utils.auth.providers.opa.Provider()
        else:
    ...
    async def authenticate_request(
        ...
        if self._basic_auth_configured():
            ...
            iguazio_client = server.api.utils.clients.iguazio.AsyncClient()
