...
class TestRuns(services.api.tests.unit.conftest.MockedK8sHelper):
    ...
    async def test_delete_runs_with_resources(self, db: sqlalchemy.orm.Session):
        ...
        with pytest.raises(mlrun.errors.MLRunNotFoundError):
            services.api.crud.Runs().get_run(db, "uid", 0, project)
    ...
    def test_run_abortion_failure(self, db: sqlalchemy.orm.Session):
        ...
        with (
            ...
            services.api.crud.Runs().abort_run(db, project, run_uid, 0)
    ...
    def test_get_workflow_run_no_artifacts(self, db: sqlalchemy.orm.Session):
        ...
        with unittest.mock.patch(
            "services.api.crud.Artifacts.list_artifacts_for_producer_id",
            side_effect=Exception("Should not be called"),
        ):
            run_1 = services.api.crud.Runs().get_run(
                db,
                run_uid_1,
                iter,
                project,
            ...
            run_2 = services.api.crud.Runs().get_run(
                db,
                run_uid_2,
                iter,
                project,
