    ...
    try:
        for kind in RuntimeKinds.runtime_with_handlers():
            try:
                ...
                runtime_handler.monitor_runs(get_db(), db_session, config.httpdb.projects.iguazio_access_key)
            except Exception as exc:
                logger.warning(
...
def _cleanup_runtimes():
    ...
    try:
        for kind in RuntimeKinds.runtime_with_handlers():
            try:
                runtime_handler = get_runtime_handler(kind)
                runtime_handler.delete_resources(get_db(), db_session, config.httpdb.projects.iguazio_access_key)
            except Exception as exc:
                logger.warning(
