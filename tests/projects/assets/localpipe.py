...
def my_pipe(param1=0):
    ...
    run2 = mlrun.run_function("tstfunc", handler="func2", params={"x": run1.outputs["accuracy"]})
    print(run2.to_yaml())

    # hack to return run result to the test for assertions
    mlrun.projects.pipeline_context._test_result = run2
    mlrun.projects.pipeline_context._artifact_path = mlrun.projects.pipeline_context.workflow_artifact_path


def args_pipe(param1=0, param2=None):
    run = mlrun.run_function("tstfunc", handler="func3", params={"p1": param1, "p2": param2})
