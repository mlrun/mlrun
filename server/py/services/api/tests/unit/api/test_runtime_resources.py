...
def test_list_runtime_resources_filter_by_kind(
    ...
    _mock_filter_project_resources_by_permissions(monkeypatch)
    runtime_handler = services.api.runtime_handlers.get_runtime_handler(filtered_kind)
...
def _mock_runtime_handlers_delete_resources(
    ...
    def _assert_delete_resources_label_selector(
        ...
        if allowed_projects:
            assert (
                services.api.api.endpoints.runtime_resources._generate_label_selector_for_allowed_projects(
                    allowed_projects
                )
                in label_selector
            )
