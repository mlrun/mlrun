...
import server.py.services.api.constants
...
class Runs(
    ...
    def store_run(
        ...
        server.py.services.api.api.utils.mask_notification_params_on_task(
            data, server.py.services.api.constants.MaskOperations.REDACT
        )

        server.py.services.api.utils.singletons.db.get_db().store_run(
            db_session,
            data,
            uid,
            project,
    ...
    def update_run(
        ...
        if (
            ...
            current_run = server.py.services.api.utils.singletons.db.get_db().read_run(
        ...
        server.py.services.api.utils.singletons.db.get_db().update_run(
    ...
    def get_run(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().read_run(
            db_session, uid, project, iter
        )
    ...
    def list_runs(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().list_runs(
            session=db_session,
            name=name,
            uid=uid,
            project=project,
    ...
    async def delete_run(
        ...
        project = project or mlrun.mlconf.default_project
        try:
            run = server.py.services.api.utils.singletons.db.get_db().read_run(
                db_session, uid, project, iter
            )
        ...
        if runtime_kind in mlrun.runtimes.RuntimeKinds.runtime_with_handlers():
            runtime_handler = (
                server.py.services.api.runtime_handlers.get_runtime_handler(
                    runtime_kind
                )
            )
            if runtime_handler.are_resources_coupled_to_run_object():
                runtime_handler.delete_runtime_object_resources(
                    server.py.services.api.utils.singletons.db.get_db(),
                    db_session,
                    object_id=uid,
        ...
        server.py.services.api.utils.singletons.db.get_db().del_run(
            db_session, uid, project, iter
        )
    ...
    async def _post_delete_run(project, uid):
        if (
            ...
            await server.py.services.api.crud.Logs().delete_run_logs(project, uid)
        else:
            await run_in_threadpool(
                server.py.services.api.crud.Logs().delete_run_logs_legacy,
                project,
                uid,
            )
