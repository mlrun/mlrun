...
class _OpenAIEndpointGroup(abc.ABC):
    ...
    _bm_cache: dict[str, endpoint_mapping.BodyMappings] = {}
    ...
    @classmethod
    def _cached_bm(cls, key: str, builder) -> endpoint_mapping.BodyMappings:
        """Return a deep copy of the cached BodyMappings for *key*, building it on first access.

        :param key: Unique name for this mapping within the class.
        :param builder: Zero-argument callable that constructs and returns the BodyMappings.
        :return: A deep copy of the cached BodyMappings instance.
        """
        if key not in cls._bm_cache:
            cls._bm_cache[key] = builder()
        return copy.deepcopy(cls._bm_cache[key])
...
class ResponsesEndpoints(_OpenAIEndpointGroup):
    ...
    @classmethod
    def _create_input_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.background", destination_path="background")
            bm.add_mapping(
                "$.context_management", destination_path="context_management"
            )
            ...
            bm.add_mapping(
                "$.parallel_tool_calls", destination_path="parallel_tool_calls"
            )
            bm.add_mapping(
        ...
        return cls._cached_bm("create_input", build)

    @classmethod
    def _response_output_bm(cls) -> endpoint_mapping.BodyMappings:
        ...
        def build():
            ...
            bm.add_mapping(
                "$.created_at", destination_path="created_at", mandatory=True
            )
            ...
            bm.add_mapping(
                "$.temperature", destination_path="temperature", mandatory=True
            )
            bm.add_mapping(
                "$.tool_choice", destination_path="tool_choice", mandatory=True
            )
        ...
        return cls._cached_bm("response_output", build)

    @classmethod
    def _compact_input_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.model", destination_path="model", mandatory=True)
            bm.add_mapping("$.input", destination_path="input")
        ...
        return cls._cached_bm("compact_input", build)

    @classmethod
    def _input_tokens_input_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.conversation", destination_path="conversation")
            bm.add_mapping("$.input", destination_path="input")
        ...
        return cls._cached_bm("input_tokens_input", build)

    @classmethod
    def _input_tokens_output_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping(
        ...
        return cls._cached_bm("input_tokens_output", build)

    @classmethod
    def _input_items_output_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
        ...
        return cls._cached_bm("input_items_output", build)

    @classmethod
    def _compact_output_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.id", destination_path="id", mandatory=True)
            bm.add_mapping("$.object", destination_path="object", mandatory=True)
...
class ChatCompletionsEndpoints(_OpenAIEndpointGroup):
    ...
    @classmethod
    def _chat_input_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            ...
            bm.add_mapping(
                "$.web_search_options", destination_path="web_search_options"
            )
            return bm

        return cls._cached_bm("chat_input", build)

    @classmethod
    def _list_chat_output_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.data", destination_path="data", mandatory=True)
            bm.add_mapping("$.first_id", destination_path="first_id", mandatory=True)
        ...
        return cls._cached_bm("list_chat_output", build)

    @classmethod
    def _delete_chat_output_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.id", destination_path="id", mandatory=True)
        ...
        return cls._cached_bm("delete_chat_output", build)

    @classmethod
    def _update_chat_input_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.metadata", destination_path="metadata", mandatory=True)
            return bm

        return cls._cached_bm("update_chat_input", build)

    @classmethod
    def _chat_completion_output_bm(cls) -> endpoint_mapping.BodyMappings:
        def build():
            bm = endpoint_mapping.BodyMappings()
            bm.add_mapping("$.id", destination_path="id", mandatory=True)
