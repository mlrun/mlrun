...
from server.py.services.api.api.utils import get_obj_path, get_secrets, log_and_raise
...
def get_filestat(
    ...
    return server.py.services.api.crud.Files().get_filestat(
        auth_info, path, schema, user
    )
...
async def get_filestat_with_project_secrets(
    ...
    return await run_in_threadpool(
        server.py.services.api.crud.Files().get_filestat,
        auth_info,
        path,
        schema,
        user,
...
async def _verify_and_get_project_secrets(project, auth_info):
    ...
    secrets_data = await run_in_threadpool(
        server.py.services.api.crud.Secrets().list_project_secrets,
        project,
        mlrun.common.schemas.SecretProviderName.kubernetes,
