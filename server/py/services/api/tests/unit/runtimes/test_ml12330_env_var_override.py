...
#
# ML-12330: Regression tests verifying that user-set plain env vars are NOT
# overridden by global/project secrets in add_k8s_secrets_to_spec, while
# preserving the project > global secret priority for secret-injected vars.
...
class TestML12330EnvVarOverride:
    """Regression tests for ML-12330: user plain env vars must not be overridden by secrets."""

    def test_user_env_var_not_overridden_by_global_secret(self):
        ...
        env_var = self._find_env_var(runtime, env_var_name)
        assert env_var is not None, (
            f"Env var '{env_var_name}' not found in runtime spec"
        )
        assert env_var.value == user_value, (
            f"ML-12330 REGRESSION: User's env var '{env_var_name}' was overridden "
        ...
        other_env = self._find_env_var(runtime, "AWS_SECRET_ACCESS_KEY")
    ...
    def test_user_env_var_not_overridden_by_project_secret(self):
        ...
        original_auto_add = (
            mlconf.secret_stores.kubernetes.auto_add_project_secrets
        )
        try:
            mlconf.secret_stores.kubernetes.global_function_env_secret_name = ""
        ...
        env_var = self._find_env_var(runtime, secret_key)
        assert env_var is not None
        assert env_var.value == user_value, (
            f"ML-12330 REGRESSION: User's env var '{secret_key}' was overridden "
            f"by project secret. Expected value='{user_value}', "
            f"got value={env_var.value!r}, value_from={env_var.value_from!r}."
        )

        # OTHER_SECRET was NOT set by user, so it should come from project secret
        other_env = self._find_env_var(runtime, "OTHER_SECRET")
        assert other_env is not None, (
            "OTHER_SECRET should be injected from project secret"
        )
        assert other_env.value_from is not None, (
            "OTHER_SECRET should be a secretKeyRef (not user-set)"
        )

    def test_project_secret_overrides_global_secret_for_same_key(self):
        """When both global and project secrets share a key, project must win.

        This is the pre-existing priority: project > global. The ML-12330 fix
        must NOT break this -- the guard only skips injection for user-set
        plain-value env vars, not for secret-injected vars.
        """
        from services.api.runtime_handlers.base import BaseRuntimeHandler
        ...
        env_var = self._find_env_var(runtime, shared_key)
        assert env_var is not None, (
            f"Env var '{shared_key}' not found in runtime spec"
        )
    ...
    def test_user_plain_var_wins_over_both_global_and_project_secrets(self):
    ...
    @staticmethod
    def _find_env_var(runtime, name):
