...
| ID    |Description                                                                 |
|-------|----------------------------------------------------------------------------|
|ML-9326| MLRun supports Python 3.11 and KFP 2.x server (and continues to support KFP 1.8 server depending on the Iguazio version). MLRun v1.9.0 CE supports KFP 2.x.<ul><li>If you are using Python 3.11 then you need to use `engine="remote"`.</li><li>Client code and workflow code and syntax (DSL) is still the KFP 1.8 syntax. Working with the newer KFP 2.x syntax is not yet supported by MLRun. KFP workflows written with KFP 2.0 syntax/APIs and run from outside MLRun are not visible in the MLRun UI. [See more about the upcoming migration to KFP v2](https://www.kubeflow.org/docs/components/pipelines/user-guides/migration/)</li><li>As in v1.8.0, KFP is not pre-installed on images such as `mlrun/mlrun`. The image `mlrun/mlrun-kfp` includes KFP, but works with py3.9.</li><li>You can install KFP manually (`pip install kfp`), for example, to run KFP pipelines locally using the KFP 1.8 client, and thereby requiring Python 3.9.</li><li>protobuf compilation is not needed anymore for python 3.11 environments.</li></ul>
|
