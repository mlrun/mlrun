...
class TestNuclioRuntime(TestMLRunSystemModelMonitoring):
    ...
    def test_generate_event_through_api_step(self):
        ...
        graph = function.set_topology("flow", engine="async")
        endpoint_path = f"projects/{self.project_name}/events/2"
        error_message = f"post event {self.project_name}/events/2"
        graph.to(
            MLRunAPIRemoteStep(
                method="POST",
                path=endpoint_path,
