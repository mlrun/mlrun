    ...
    if project != "*":
        ...
        while page_token is not None:
            response = kfp_client._run_api.list_runs(
                page_token=page_token, page_size=mlrun.api.schemas.PipelinesPagination.max_page_size
            )
        ...
        response = kfp_client._run_api.list_runs(
            page_token=page_token, page_size=page_size or mlrun.api.schemas.PipelinesPagination.default_page_size, sort_by=sort_by, filter=filter_
        )
...
def _format_runs(runs: typing.List[dict], format_: mlrun.api.schemas.Format = mlrun.api.schemas.Format.metadata_only) -> typing.List[dict]:
        ...
        for index, argument in enumerate(command):
            if (argument == '-f' or argument == '--func-url') and index + 1 < len(command):
                ...
                if function_url.startswith("db://"):
                    project, _, _, _ = mlrun.utils.helpers.parse_function_uri(function_url[len("db://"):])
                    if project:
                        return project
