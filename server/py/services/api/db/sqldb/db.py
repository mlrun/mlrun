...
from server.py.services.api.db.sqldb.helpers import (
...
from server.py.services.api.db.sqldb.models import (
    Artifact,
    ArtifactV2,
    BackgroundTask,
    Base,
...
from server.py.services.api.db.sqldb.models.common import (
    _labeled,
    _tagged,
    _with_notifications,
)
...
class SQLDB(DBInterface):
    ...
    def tag_artifacts(
        ...
        for artifact in artifacts:
            ...
            tag = server.py.services.api.db.session.run_function_with_new_db_session(
                _get_tag
            )
            if not tag:
                # create the new tag
    ...
    def list_projects(
        ...
        for project_record in project_records:
            if format_ == mlrun.common.schemas.ProjectsFormat.name_only:
                ...
                projects.append(
                    server.py.services.api.utils.helpers.minimize_project_schema(
    ...
    def store_background_task(
        ...
        error = server.py.services.api.db.sqldb.helpers.ensure_max_length(error)
