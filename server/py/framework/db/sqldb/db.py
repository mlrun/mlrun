...
class SQLDB(DBInterface):
    ...
    def _find_model_endpoints(
        ...
        if function_name:
            query = query.filter(ModelEndpoint.function.name == function_name)
        if function_tag:
            query = query.filter(
                ModelEndpoint.function.tags.any(Function.Tag.name == function_tag)
            )
        if model_name:
            query = query.filter(ModelEndpoint.model.name == model_name)
        if model_tag:
            query = query.filter(
                ModelEndpoint.model.tags.any(ArtifactV2.Tag.name == model_tag)
            )
        if uids:
            query = query.filter(ModelEndpoint.uid.in_(uids))
        if top_level:
    ...
    def _create_mep_record_to_store(
        ...
        if model_path and mlrun.datastore.is_store_uri(model_path):
            ...
            model_record = self.read_artifact(
                session,
                key=key,
                tag=tag,
                iter=iteration,
            ...
            model_labels = {label.name: label for label in model_record.labels}
            model_endpoint.metadata.labels.update(model_labels)
        # get model_id
        mep = ModelEndpoint(
