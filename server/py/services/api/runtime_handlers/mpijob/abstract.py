...
from server.py.services.api.runtime_handlers import KubeRuntimeHandler
...
class AbstractMPIJobRuntimeHandler(KubeRuntimeHandler, abc.ABC):
    ...
    def get_job(self, name, namespace=None):
        ...
        try:
            resp = server.py.services.api.utils.singletons.k8s.get_k8s_helper().crdapi.get_namespaced_custom_object(
    ...
    def _submit_mpijob(self, job, namespace=None):
        ...
        try:
            resp = server.py.services.api.utils.singletons.k8s.get_k8s_helper().crdapi.create_namespaced_custom_object(
                mpi_group,
                mpi_version,
                namespace=namespace,
                plural=mpi_plural,
