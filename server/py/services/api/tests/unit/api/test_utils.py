...
@pytest.mark.asyncio
async def test_delete_function_deletes_code_artifact(db: sqlalchemy.orm.Session):
    """Verify that _delete_function deletes a store:// code artifact when delete_code_artifact=True."""
    project = "my_project"
    function_name = "my_func"
    artifact_key = "my_func_code"
    ...
    # Verify the artifact exists
    artifacts = services.api.crud.Artifacts().list_artifacts(
        db, project=project, name=artifact_key, tag="*"
    )
    assert len(artifacts) == 1

    # Store a function that references the code artifact via store:// URI
    store_uri = f"store://artifacts/{project}/{artifact_key}"
    function = {
        "kind": "job",
        "metadata": {"name": function_name, "tag": "latest"},
        "spec": {"build": {"source": store_uri}},
        "status": {},
    }
    services.api.crud.Functions().store_function(
        db, project=project, function=function, name=function_name, tag="latest"
    )

    # Delete the function with delete_code_artifact=True
    await framework.api.utils._delete_function(
        db,
        project,
        function_name,
        mlrun.common.schemas.AuthInfo(),
        background_task_name="task-123",
        delete_code_artifact=True,
    )

    # Verify the code artifact was deleted
    artifacts = services.api.crud.Artifacts().list_artifacts(
        db, project=project, name=artifact_key, tag="*"
    )
    assert len(artifacts) == 0
...
@pytest.mark.asyncio
async def test_delete_function_skips_code_artifact_when_flag_false(
    db: sqlalchemy.orm.Session,
):
    """Verify that _delete_function does NOT delete the code artifact when delete_code_artifact=False."""
    project = "my_project"
    function_name = "my_func"
    artifact_key = "my_func_code"
    ...
    # Delete the function with delete_code_artifact=False
    await framework.api.utils._delete_function(
        db,
        project,
        function_name,
        mlrun.common.schemas.AuthInfo(),
        background_task_name="task-456",
        delete_code_artifact=False,
    )

    # Verify the code artifact still exists
    artifacts = services.api.crud.Artifacts().list_artifacts(
        db, project=project, name=artifact_key, tag="*"
    )
    assert len(artifacts) == 1
...
@pytest.mark.asyncio
async def test_delete_function_code_artifact_tolerates_failure():
    """Verify that code artifact deletion failure does not prevent function deletion."""
    with (
        patch.object(
            services.api.crud.Functions,
            "list_functions",
            return_value=[
                {
                    "kind": "job",
                    "metadata": {"name": "my_func", "tag": "latest"},
                    "spec": {
                        "build": {"source": "store://artifacts/my_project/my_func_code"}
                    },
                    "status": {},
                }
            ],
        ...
        await framework.api.utils._delete_function(
            ...
            "my_func",
            mlrun.common.schemas.AuthInfo(),
            background_task_name="task-789",
            delete_code_artifact=True,
        )

        # Function deletion should still proceed despite artifact deletion failure
