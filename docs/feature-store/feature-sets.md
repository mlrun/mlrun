...
>  **Limitation**: Do not ingest data with column names that start with `t_`. This is reserved for internal use, and the data does not ingest correctly. See also [Attribute name restrictions](https://www.iguazio.com/docs/latest-release/data-layer/objects/attributes/#attribute-names).
