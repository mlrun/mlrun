...
async def abort_run(
    ...
    async with abort_run_background_tasks_cache.get_or_create_locked(
        key=uid,
        ttl=cache_ttl,
        ...
        if not created:
            ...
            background_task_indicator = await abort_run_background_tasks_cache.create(
...
class CachedAbortRunBackgroundTask(server.api.utils.cache.CachedObject):
    def __init__(
        ...
        super().__init__(ttl, lock)
        self.background_task_id = background_task_id or str(uuid.uuid4())


abort_run_background_tasks_cache = server.api.utils.cache.Cache(
