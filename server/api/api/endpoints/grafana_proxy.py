...
import server.api.crud.model_monitoring.grafana
...
NAME_TO_SEARCH_FUNCTION_DICTIONARY = {
    "list_projects": server.api.crud.model_monitoring.grafana.grafana_list_projects,
}
NAME_TO_QUERY_FUNCTION_DICTIONARY = {
    "list_endpoints": server.api.crud.model_monitoring.grafana.grafana_list_endpoints,
    "individual_feature_analysis": server.api.crud.model_monitoring.grafana.grafana_individual_feature_analysis,
    "overall_feature_analysis": server.api.crud.model_monitoring.grafana.grafana_overall_feature_analysis,
    "incoming_features": server.api.crud.model_monitoring.grafana.grafana_incoming_features,
}
...
def grafana_proxy_model_endpoints_check_connection(
    ...
    if not mlrun.mlconf.is_ce_mode():
        server.api.crud.model_monitoring.helpers.get_access_key(auth_info)
...
async def grafana_proxy_model_endpoints_search(
    ...
    query_parameters = server.api.crud.model_monitoring.grafana.parse_search_parameters(
        body
    )
    server.api.crud.model_monitoring.grafana.validate_query_parameters(
...
async def grafana_proxy_model_endpoints_query(
    ...
    query_parameters = (
        server.api.crud.model_monitoring.grafana.drop_grafana_escape_chars(
            query_parameters
        )
    )
