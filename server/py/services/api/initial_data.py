...
import server.py.services.api.db.sqldb.db
...
import server.py.services.api.utils.db.mysql
...
from server.py.services.api.db.init_db import init_db
from server.py.services.api.db.session import close_session, create_session
...
def init_data(
    ...
    mysql_util = server.py.services.api.utils.db.mysql.MySQLUtil(logger)
    ...
    if is_backup_needed:
        ...
        db_backup = server.py.services.api.utils.db.backup.DBBackupUtil()
...
def update_default_configuration_data():
    ...
    db_session = create_session()
    try:
        db = server.py.services.api.db.sqldb.db.SQLDB()
...
def _resolve_needed_operations(
    alembic_util: server.py.services.api.utils.db.alembic.AlembicUtil,
...
def _create_alembic_util() -> server.py.services.api.utils.db.alembic.AlembicUtil:
    alembic_config_file_name = "alembic.ini"
    if server.py.services.api.utils.db.mysql.MySQLUtil.get_mysql_dsn_data():
...
def _perform_version_2_data_migrations(
    db: server.py.services.api.db.sqldb.db.SQLDB, db_session: sqlalchemy.orm.Session
):
...
def _align_runs_table(
    ...
    for run in runs:
        ...
        run.start_time = (
            server.py.services.api.db.sqldb.helpers.run_start_time(run_dict)
            or run.start_time
        )
...
def _delete_default_hub_source(db_session: sqlalchemy.orm.Session):
    ...
    default_record = (
        db_session.query(server.py.services.api.db.sqldb.models.HubSource)
        .filter(
            server.py.services.api.db.sqldb.models.HubSource.index
...
def _migrate_artifacts_table_v2(
    ...
    total_artifacts_count = db._query(
        db_session, server.py.services.api.db.sqldb.models.Artifact
    ).count()
    ...
    db.delete_table_records(
        db_session,
        server.py.services.api.db.sqldb.models.Artifact.Label,
        raise_on_not_exists=False,
    )
    db.delete_table_records(
        db_session,
        server.py.services.api.db.sqldb.models.Artifact.Tag,
        raise_on_not_exists=False,
    )
    db.delete_table_records(
        db_session,
...
def _migrate_artifacts_batch(
    ...
    query = db._query(db_session, server.py.services.api.db.sqldb.models.Artifact)
    if last_migrated_artifact_id > 0:
        # skip the artifacts that were already migrated
        query = query.filter(
            server.py.services.api.db.sqldb.models.Artifact.id
            > last_migrated_artifact_id
        )

    query = query.order_by(server.py.services.api.db.sqldb.models.Artifact.id).limit(
        batch_size
    )
    ...
    for artifact in artifacts:
        new_artifact = server.py.services.api.db.sqldb.models.ArtifactV2()
...
def _migrate_artifact_tags(
    db_session: sqlalchemy.orm.Session,
    old_id_to_artifact: dict[
        typing.Any, server.py.services.api.db.sqldb.models.ArtifactV2
    ],
):
    ...
    old_tags = (
        db_session.query(server.py.services.api.db.sqldb.models.Artifact.Tag)
        .filter(
            server.py.services.api.db.sqldb.models.Artifact.Tag.obj_id.in_(
    ...
    for old_tag in old_tags:
        ...
        new_tag = server.py.services.api.db.sqldb.models.ArtifactV2.Tag(
...
def _mark_best_iteration_artifacts(
    ...
    link_artifacts = (
        db_session.query(server.py.services.api.db.sqldb.models.Artifact)
        .filter(
            server.py.services.api.db.sqldb.models.Artifact.id.in_(link_artifact_ids)
        )
        .all()
    )
    ...
    for link_artifact in link_artifacts:
        ...
        # get the artifacts attached to the link artifact
        query = db._query(
            db_session, server.py.services.api.db.sqldb.models.ArtifactV2
        ).filter(
            server.py.services.api.db.sqldb.models.ArtifactV2.key == link_artifact_key,
            server.py.services.api.db.sqldb.models.ArtifactV2.iteration
            == link_iteration,
        )
        if link_tree:
            query = query.filter(
                server.py.services.api.db.sqldb.models.ArtifactV2.producer_id
                == link_tree
            )
