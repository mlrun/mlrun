...
class Functions(
    ...
    def store_function(
        ...
        if auth_info:
            ...
            server.py.services.api.api.utils.ensure_function_auth_and_sensitive_data_is_masked(
                function_obj, auth_info, allow_empty_access_key=True
            )
            function = function_obj.to_dict()

        return server.py.services.api.utils.singletons.db.get_db().store_function(
            db_session,
            function,
            name,
            project,
    ...
    def get_function(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().get_function(
    ...
    def delete_function(
        ...
        return server.py.services.api.utils.singletons.db.get_db().delete_function(
    ...
    def list_functions(
        ...
        if labels is None:
            labels = []
        return server.py.services.api.utils.singletons.db.get_db().list_functions(
            session=db_session,
            name=name,
            project=project,
            tag=tag,
    ...
    def get_function_status(
        ...
        resource = server.py.services.api.runtime_handlers.runtime_resources_map.get(
            kind
        )
