...
def test_grafana_endpoint_features(db: Session, client: TestClient):
    ...
    response = client.post(
        ...
        headers={"X-V3io-Session-Key": _get_access_key()},
        json={
            "targets": [{"target": "project=test;endpoint_id=test.test_id;target_endpoint=endpoint_features"}]
        },
    )
...
def test_grafana_endpoint_features_missing_field_doesnt_fail(db: Session, client: TestClient):
    endpoint_data = {
        "timestamp": "2021-02-28 21:02:58.642108",
        "project": "test",
        "model": "test-model",
