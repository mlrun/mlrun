...
func StartServer() error {
	...
	monitoringInterval := flag.String("monitoring-interval", common.GetEnvOrDefaultString("MLRUN_LOG_COLLECTOR_MONITORING_INTERVAL", "30s"), "Periodic interval for monitoring the gorooutines collecting logs")
	bufferPoolSize := flag.Int("buffer-pool-size", common.GetEnvOrDefaultInt("MLRUN_LOG_COLLECTOR_BUFFER_POOL_SIZE", 512), "Number of buffers in the buffer pool")
	...
	monitoringInterval := flag.String("monitoring-interval", common.GetEnvOrDefaultString("MLRUN_LOG_COLLECTOR_MONITORING_INTERVAL", "10s"), "Periodic interval for monitoring the gorooutines collecting logs")
	logCollectionbufferPoolSize := flag.Int("log-collection-buffer-pool-size", common.GetEnvOrDefaultInt("MLRUN_LOG_COLLECTOR_LOG_COLLECTION_BUFFER_POOL_SIZE", 100), "Number of buffers in the buffer pool for collecting logs")
	getLogsBufferPoolSize := flag.Int("get-logs-buffer-pool-size", common.GetEnvOrDefaultInt("MLRUN_LOG_COLLECTOR_GET_LOGS_BUFFER_POOL_SIZE", 100), "Number of buffers in the buffer pool for getting logs")
