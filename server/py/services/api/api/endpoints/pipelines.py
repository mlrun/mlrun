...
from server.py.services.api.api.utils import log_and_raise
...
async def list_pipelines(
    ...
    if server.py.services.api.utils.singletons.k8s.get_k8s_helper(
        ...
        total_size, next_page_token, runs = await run_in_threadpool(
            server.py.services.api.crud.Pipelines().list_pipelines,
            db_session,
            project,
            namespace,
            sort_by,
...
async def get_pipeline(
    ...
    pipeline = await run_in_threadpool(
        server.py.services.api.crud.Pipelines().get_pipeline,
        db_session,
        run_id,
        project,
        namespace,
...
async def _create_pipeline(
    ...
    run = await run_in_threadpool(
        server.py.services.api.crud.Pipelines().create_pipeline,
        experiment_name,
        run_name,
        content_type,
        data,
...
def _try_resolve_project_from_body(
    ...
    return (
        server.py.services.api.crud.Pipelines().resolve_project_from_workflow_manifest(
            workflow_manifest
        )
    )
