...
def test_list_completed_runs(db: Session, client: TestClient):
    ...
    runs = [{"metadata": {"name": "completed-run", "uid": "completed-uid", "project": project},
            "status": {"state": mlrun.common.runtimes.constants.RunStates.completed}},
            {"metadata": {"name": "running-run", "uid": "running-uid", "project": project},
            "status": {"state": mlrun.common.runtimes.constants.RunStates.running}},
            {"metadata": {"name": "error-run", "uid": "error-uid", "project": project},
            "status": {"state": mlrun.common.runtimes.constants.RunStates.error}},
            {"metadata": {"name": "aborted-run", "uid": "aborted-uid", "project": project},
            "status": {"state": mlrun.common.runtimes.constants.RunStates.aborted}}]

    # Store runs in the database
    for run in runs:
        services.api.crud.Runs().store_run(db, run, run["metadata"]["uid"], project=project)
...
def test_completed_runs_start_time_from(db: Session, client: TestClient):
    ...
    run_early_dict = {
        ...
        "status": {"state": mlrun.common.runtimes.constants.RunStates.completed, "start_time": earlier.isoformat()},
    }
    run_late_dict = {
        "metadata": {"name": "late-run", "uid": "late-uid", "project": project},
        "status": {"state": mlrun.common.runtimes.constants.RunStates.completed, "start_time": later.isoformat()},
    }

    services.api.crud.Runs().store_run(db, run_early_dict, run_early_dict["metadata"]["uid"], project=project)
    services.api.crud.Runs().store_run(db, run_late_dict, run_late_dict["metadata"]["uid"], project=project)

    # Query with start_time_from set to just after the earlier run
    start_time_from = (earlier + timedelta(minutes=1)).isoformat()
    response = client.get(
        f"/projects/{project}/completed_runs", params={"start_time_from": start_time_from}
    )
    assert response.status_code == HTTPStatus.OK.value
    runs = response.json()["runs"]
    assert len(runs) == 1
...
def test_abort_aborted_run_with_background_task(
    ...
    with unittest.mock.patch.object(
        ...
        background_task_1 = framework.utils.background_tasks.ProjectBackgroundTasksHandler().get_background_task(
        ...
        background_task_2 = framework.utils.background_tasks.ProjectBackgroundTasksHandler().get_background_task(
...
def _store_run(db, uid, project="some-project", name="run-name"):
    ...
    if not uid:
        uid = str(uuid.uuid4())
    return services.api.crud.Runs().store_run(
