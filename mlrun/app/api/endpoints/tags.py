...
def tag_objects(
    ...
    get_db_instance().tag_objects(db_session, objs, project, name)
    return {
        "project": project,
        "name": name,
        "count": len(objs),
...
def del_tag(
        ...
        name: str,
        db_session: Session = Depends(deps.get_db_session)):
    count = get_db_instance().del_tag(db_session, project, name)
    return {
        "project": project,
        "name": name,
        "count": count,
...
def list_tags(
        project: str,
        db_session: Session = Depends(deps.get_db_session)):
    tags = get_db_instance().list_tags(db_session, project)
    return {
        "project": project,
        "tags": tags,
    }
...
def get_tagged(
        ...
        name: str,
        db_session: Session = Depends(deps.get_db_session)):
    objs = get_db_instance().find_tagged(db_session, project, name)
    return {
        "project": project,
        "tag": name,
