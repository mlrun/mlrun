...
class AuthVerifier(metaclass=mlrun.utils.singleton.Singleton):
    ...
    def __init__(self) -> None:
        ...
        if mlrun.mlconf.httpdb.authorization.mode == "none":
            self._auth_provider = services.api.utils.auth.providers.nop.Provider()
        elif mlrun.mlconf.httpdb.authorization.mode == "opa":
            self._auth_provider = services.api.utils.auth.providers.opa.Provider()
        else:
    ...
    async def authenticate_request(
        ...
        if self._basic_auth_configured():
            ...
            iguazio_client = services.api.utils.clients.iguazio.AsyncClient()
