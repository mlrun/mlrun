...
def function_status():
    ...
    app.logger.info('function_status:\n{}'.format(data))
...
def build_status():
    ...
    app.logger.info('get pod {} status'.format(pod))
    state = app.k8s.get_pod_status(pod)
    app.logger.info('pod state={}'.format(state))

    if state == 'succeeded':
        app.logger.info('build completed successfully')
        state = 'ready'
    if state in ['failed', 'error']:
        app.logger.error('build {}, watch the build pod logs: {}'.format(
            state, pod))
    ...
    app.db.store_function(fn, name, project, tag)
...
def get_function(project, name):
    tag = request.args.get('tag', '')
    func = app.db.get_function(name, project, tag)
...
def list_functions():
    ...
    out = app.db.list_functions(name, project, tag, labels)
    return jsonify(
        ok=True,
        funcs=list(out),
    )
