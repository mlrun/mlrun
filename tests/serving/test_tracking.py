...
class TestMonitoringPreProcessorStreamingAggregation:
    ...
    def test_do_handles_streaming_error_event(self, rundb_mock):
        """Test that a stream-collected event carrying an error is tracked correctly.

        When a streaming invocation fails (e.g. MockModelProvider raises on ERROR
        keyword), the Collector emits a stream_collected event whose aggregated body
        is a dict with an 'error' key. The MonitoringPreProcessor should preserve the
        error and set outputs/output_schema to None.
        """
        function = mlrun.new_function("test-stream-error", kind="serving")
        graph = function.set_topology("flow", engine="async")
        model_runner_step = ModelRunnerStep(name="my_runner")
        model_runner_step.add_model(
            model_class="SimpleTestModel",
        ...
        try:
            event = storey.Event(
                body=[{"error": "Mock error triggered by ERROR keyword in message"}],
            )
            event._metadata = {
                "model_runner_name": "my_runner",
                "when": "2026-01-01 00:00:00.000000+00:00",
                "microsec": 1000,
                "inputs": {"question": "ERROR - this should fail"},
            }
            event.stream_collected = True
            ...
            assert len(result.body) == 1
            monitoring_event = result.body[0]
            assert "Mock error triggered by ERROR keyword" in monitoring_event["error"]
            assert monitoring_event["resp"]["outputs"] is None
            assert monitoring_event["resp"]["output_schema"] is None
        finally:
            server.wait_for_completion()

    def test_streaming_error_event_through_error_extractor(self, rundb_mock):
        """Reproduce the system-test failure where streaming error events fail
        to be recorded in the TSDB errors table.

        We feed the MonitoringPreProcessor output for a streaming error event
        through ErrorExtractor (the stream-processing-graph step that prepares
        events for the errors TSDB table) and verify it produces a valid record.
        """
        ...
        function = mlrun.new_function("test-stream-error-extract", kind="serving")
        graph = function.set_topology("flow", engine="async")
        model_runner_step = ModelRunnerStep(name="my_runner")
        model_runner_step.add_model(
            model_class="SimpleTestModel",
        ...
        try:
            # Build a stream-collected error event (as Collector would emit)
            event = storey.Event(
                body=[{"error": "Mock error triggered by ERROR keyword in message"}],
            )
            event._metadata = {
            ...
            # Verify the monitoring event has the fields ErrorExtractor expects
            assert monitoring_event.get("error") is not None, "error field must be set"
            assert (
                monitoring_event.get("when") is not None
            ), f"when field must be set, got: {monitoring_event.get('when')}"
            # endpoint_id is None in mock tests (no real model endpoint);
            # in the system test it is populated from the DB.

            # Feed the monitoring event through ErrorExtractor.
            # Patch endpoint_id to simulate the system test path where
            # the model endpoint is registered.
            monitoring_event["endpoint_id"] = "test-endpoint-uid"
            extractor = ErrorExtractor()
            error_record = extractor.do(monitoring_event)

            # Verify the error record has the expected structure for tsdb_error
            assert error_record["model_error"] == (
                "Mock error triggered by ERROR keyword in message"
            )
            assert error_record["error_type"] == "infer_error"
            assert error_record["error_count"] == 1.0
            assert isinstance(error_record["timestamp"], datetime)
            assert error_record["endpoint_id"] == "test-endpoint-uid"
        finally:
            server.wait_for_completion()
