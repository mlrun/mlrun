...
def mask_notification_params_on_task(
    task: dict,
    action: services.api.constants.MaskOperations,
):
    """
...
def _notification_params_mask_op(
    ...
    return {
        services.api.constants.MaskOperations.CONCEAL: _conceal_notification_params_with_secret,
        services.api.constants.MaskOperations.REDACT: _redact_notification_params,
    }[action]
...
def unmask_notification_params_secret_on_task(
    db: services.api.db.base.DBInterface,
    db_session: Session,
...
def get_or_create_project_deletion_background_task(
    ...
    try:
        task = services.api.utils.background_tasks.InternalBackgroundTasksHandler().get_active_background_task_by_kind(
    ...
    return services.api.utils.background_tasks.InternalBackgroundTasksHandler().create_background_task(
        background_task_kind,
...
def verify_project_is_deleted(project_name, auth_info):
    def _verify_project_is_deleted():
        try:
            project = services.api.db.session.run_function_with_new_db_session(
