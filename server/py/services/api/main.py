...
import server.py.services.api.middlewares
...
from server.py.services.api.api.api import api_router, api_v2_router
from server.py.services.api.db.session import close_session, create_session
from server.py.services.api.runtime_handlers import get_runtime_handler
from server.py.services.api.utils.periodic import (
    cancel_all_periodic_functions,
    cancel_periodic_function,
    run_function_periodically,
)
from server.py.services.api.utils.singletons.db import get_db, initialize_db
from server.py.services.api.utils.singletons.k8s import get_k8s_helper
from server.py.services.api.utils.singletons.logs_dir import initialize_logs_dir
from server.py.services.api.utils.singletons.project_member import (
    get_project_member,
    initialize_project_member,
)
from server.py.services.api.utils.singletons.scheduler import (
    get_scheduler,
...
app.add_middleware(
    server.py.services.api.middlewares.EnsureBackendVersionMiddleware,
    backend_version=config.version,
)
app.add_middleware(
    server.py.services.api.middlewares.UiClearCacheMiddleware,
    backend_version=config.version,
)
app.add_middleware(
    server.py.services.api.middlewares.RequestLoggerMiddleware, logger=logger
)
...
async def move_api_to_online():
    ...
    if (
        ...
        server.py.services.api.initial_data.update_default_configuration_data()
...
def _push_terminal_run_notifications(
    db: server.py.services.api.db.base.DBInterface, db_session
):
    """
...
async def _stop_logs_for_runs(runs: list, chunk_size: int = 10):
    ...
    for project_name, run_uids in project_to_run_uids.items():
        ...
        for chunked_run_uids in mlrun.utils.helpers.iterate_list_by_chunks(
            ...
            try:
                await server.py.services.api.utils.clients.log_collector.LogCollectorClient().stop_logs(
...
if __name__ == "__main__":
    ...
    import server.py.services.api.apiuvicorn as uvicorn
