"""Add partition_key column and back‑fill existing rows (MySQL only)"""
...
def upgrade():
    ...
    if bind.dialect.name.startswith(mlrun.common.db.dialects.Dialects.MYSQL):
        ...
        expr = interval.get_partition_expression(
        ...
        op.execute(sqlalchemy.update(tbl).values(partition_key=expr))
    else:
        logger.info(
