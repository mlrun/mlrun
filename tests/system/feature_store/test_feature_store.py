...
from ...datastore.databricks_utils import is_databricks_configured, MLRUN_ROOT_DIR, setup_dbfs_dirs, teardown_dbfs_dirs, \
    MUST_HAVE_VARIABLES
...
class TestFeatureStore(TestMLRunSystem):
    ...
    def test_ingest_with_dbfs(self, source_class, target_class, local_file_name):
        ...
        expected = pd.read_csv(local_source_path, parse_dates=["date_of_birth"]).drop(drop_column, axis=1)
        ...
        setup_dbfs_dirs(workspace=workspace, specific_test_class_dir=test_dir,
        ...
        try:
            ...
            source = CSVSource(
                "mycsv", dbfs_source_path, parse_dates=["date_of_birth"]
            ...
            target_generated_dirs = list(workspace.dbfs.list(f"dbfs://{MLRUN_ROOT_DIR}{test_dir}/{generated_uuid}"))
            if len(target_generated_dirs) > 2:
                raise Exception("target generated more then 1 folder")
            target_generated_dir_path = target_generated_dirs[0].path if not target_generated_dirs[0].path.endswith(
                f"source_{base_filename}{extension}") else target_generated_dirs[1].path
            target_file_path = f'dbfs://{target_generated_dir_path}/target_{base_filename}{extension}'
            result = pd.read_csv(target_file_path, storage_options={"instance": os.environ.get("DATABRICKS_HOST"),
                                                                    "token": os.environ.get("DATABRICKS_TOKEN")},
                                 parse_dates=["date_of_birth"])
            assert_frame_equal(expected.sort_index(axis=1), result.sort_index(axis=1), check_like=True)
        finally:
