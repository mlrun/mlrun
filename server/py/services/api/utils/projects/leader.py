...
import server.py.services.api.utils.periodic
import server.py.services.api.utils.projects.member
import server.py.services.api.utils.projects.member as project_member
import server.py.services.api.utils.projects.remotes.follower
import server.py.services.api.utils.projects.remotes.nop_follower
...
class Member(
    ...
    def _start_periodic_sync(self):
        ...
        if self._periodic_sync_interval_seconds > 0 and self._followers:
            ...
            server.py.services.api.utils.periodic.run_function_periodically(
    ...
    def _stop_periodic_sync(self):
        server.py.services.api.utils.periodic.cancel_periodic_function(
            self._sync_projects.__name__
        )

    def _sync_projects(self):
        db_session = server.py.services.api.db.session.create_session()
        try:
            ...
            server.py.services.api.db.session.close_session(db_session)
    ...
    def _initialize_follower(
        self, name: str
    ) -> server.py.services.api.utils.projects.remotes.follower.Member:
        followers_classes_map = {
            "mlrun": server.py.services.api.crud.Projects(),
            "nuclio": server.py.services.api.utils.clients.nuclio.Client(),
            # for tests
            "nop-self-leader": server.py.services.api.utils.projects.remotes.nop_follower.Member(),
            "nop": server.py.services.api.utils.projects.remotes.nop_follower.Member(),
            "nop2": server.py.services.api.utils.projects.remotes.nop_follower.Member(),
        }
