...
class FeatureStore(
    ...
    def list_feature_sets(
        ...
        return server.py.services.api.utils.singletons.db.get_db().list_feature_sets(
            db_session,
            project,
            name,
            tag,
    ...
    def list_features(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().list_features(
            db_session,
            project,
            name,
            tag,
    ...
    def list_entities(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().list_entities(
            db_session,
            project,
            name,
            tag,
    ...
    def list_feature_vectors(
        ...
        project = project or mlrun.mlconf.default_project
        return server.py.services.api.utils.singletons.db.get_db().list_feature_vectors(
            db_session,
            project,
            name,
            tag,
    ...
    def _create_object(
        ...
        if isinstance(object_, mlrun.common.schemas.FeatureSet):
            return (
                server.py.services.api.utils.singletons.db.get_db().create_feature_set(
                    db_session, project, object_, versioned
                )
            )
        elif isinstance(object_, mlrun.common.schemas.FeatureVector):
            return server.py.services.api.utils.singletons.db.get_db().create_feature_vector(
                db_session, project, object_, versioned
            )
        else:
    ...
    def _store_object(
        ...
        if isinstance(object_, mlrun.common.schemas.FeatureSet):
            return (
                server.py.services.api.utils.singletons.db.get_db().store_feature_set(
                    db_session,
                    project,
                    name,
                    object_,
            ...
            return server.py.services.api.utils.singletons.db.get_db().store_feature_vector(
                db_session,
                project,
                name,
                object_,
    ...
    def _patch_object(
        ...
        if object_schema.__name__ == mlrun.common.schemas.FeatureSet.__name__:
            return (
                server.py.services.api.utils.singletons.db.get_db().patch_feature_set(
                    db_session,
                    project,
                    name,
                    object_patch,
            ...
            return server.py.services.api.utils.singletons.db.get_db().patch_feature_vector(
                db_session,
                project,
                name,
                object_patch,
    ...
    def _get_object(
        ...
        project = project or mlrun.mlconf.default_project
        if object_schema.__name__ == mlrun.common.schemas.FeatureSet.__name__:
            return server.py.services.api.utils.singletons.db.get_db().get_feature_set(
                db_session, project, name, tag, uid
            )
        elif object_schema.__name__ == mlrun.common.schemas.FeatureVector.__name__:
            return (
                server.py.services.api.utils.singletons.db.get_db().get_feature_vector(
                    db_session, project, name, tag, uid
                )
            )
        else:
    ...
    def _list_object_type_tags(
        ...
        project = project or mlrun.mlconf.default_project
        if object_schema.__name__ == mlrun.common.schemas.FeatureSet.__name__:
            return server.py.services.api.utils.singletons.db.get_db().list_feature_sets_tags(
                db_session, project
            )
        elif object_schema.__name__ == mlrun.common.schemas.FeatureVector.__name__:
            return server.py.services.api.utils.singletons.db.get_db().list_feature_vectors_tags(
                db_session, project
            )
        else:
            raise NotImplementedError(
