...
import server.py.services.api.utils.clients.log_collector as log_collector
...
class Logs(
    ...
    def store_log(
        ...
        log_file = server.py.services.api.api.utils.log_path(project, uid)
    ...
    def delete_run_logs_legacy(
        ...
        project = project or mlrun.mlconf.default_project
        logs_path = server.py.services.api.api.utils.log_path(project, run_uid)
        if logs_path.exists():
            shutil.rmtree(str(logs_path))
    ...
    def _get_logs_legacy_method(
        ...
        if log_file_exists and source in [LogSources.AUTO, LogSources.PERSISTENCY]:
            ...
            if k8s and k8s.is_running_inside_kubernetes_cluster():
                ...
                pods = server.py.services.api.utils.singletons.k8s.get_k8s_helper().get_logger_pods(
                ...
                if pods:
                    ...
                    if pod_phase != PodPhases.pending:
                        resp = server.py.services.api.utils.singletons.k8s.get_k8s_helper().logs(
                            pod
                        )
                        if resp:
                            if size == -1:
    ...
    def log_file_exists_for_run_uid(project: str, uid: str) -> (bool, pathlib.Path):
        ...
        """
        project_logs_dir = server.py.services.api.api.utils.project_logs_path(project)
