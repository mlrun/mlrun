...
import server.py.services.api.utils.scheduler
...
from server.py.services.api.utils.runtimes.nuclio import resolve_nuclio_version
...
class MonitoringDeployment:
    ...
    def deploy_model_monitoring_batch_processing(
        ...
        if not overwrite:
            ...
            try:
                fn = server.py.services.api.crud.Functions().get_function(
                    db_session=db_session,
                    name=function_name,
                    project=project,
                )
        ...
        if not fn:
            ...
            if with_schedule:
                if not overwrite:
                    try:
                        server.py.services.api.utils.scheduler.Scheduler().get_schedule(
                            db_session=db_session,
                            project=project,
                            name=function_name,
                        )
    ...
    def _get_model_monitoring_batch_function(
        ...
        server.py.services.api.api.utils.apply_enrichment_and_validation_on_function(
            function, auth_info
        )
    ...
    def _submit_schedule_batch_job(
        ...
        server.py.services.api.api.utils.submit_run_sync(
    ...
    def _generate_schedule_and_interval_dict(
        ...
        if function_name == mm_constants.MonitoringFunctionNames.BATCH:
            ...
            (
                ...
                hours,
                days,
            ) = server.py.services.api.crud.model_monitoring.helpers.get_batching_interval_param(
                interval_list
            )
            schedule = server.py.services.api.crud.model_monitoring.helpers.convert_to_cron_string(
    ...
    def _apply_access_key_and_mount_function(
        ...
        if function_name in mm_constants.MonitoringFunctionNames.all():
            ...
            function.set_env_from_secret(
                ...
                server.py.services.api.utils.singletons.k8s.get_k8s_helper().get_project_secret_name(
                    project
                ),
                server.py.services.api.crud.secrets.Secrets().generate_client_project_secret_key(
                    server.py.services.api.crud.secrets.SecretsClientType.model_monitoring,
                    mm_constants.ProjectSecretKeys.ACCESS_KEY,
                ),
            )
            ...
            server.py.services.api.api.utils.ensure_function_has_auth_set(
                function, auth_info
            )
        return function
