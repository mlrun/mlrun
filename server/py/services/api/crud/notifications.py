...
class Notifications(
    ...
    def store_run_notifications(
        ...
        if mask_params:
            notification_objects_to_store = (
                server.py.services.api.api.utils.validate_and_mask_notification_list(
                    notification_objects, run_uid, project
                )
            )

        server.py.services.api.utils.singletons.db.get_db().store_run_notifications(
    ...
    def list_run_notifications(
        ...
        project = project or mlrun.mlconf.default_project
        return (
            server.py.services.api.utils.singletons.db.get_db().list_run_notifications(
    ...
    def delete_run_notifications(
        ...
        server.py.services.api.utils.singletons.db.get_db().delete_run_notifications(
    ...
    def set_object_notifications(
        ...
        set_notification_methods = {
            "run": {
                "factory": server.py.services.api.utils.singletons.db.get_db,
                "method_name": server.py.services.api.db.sqldb.db.SQLDB.set_run_notifications.__name__,
                "identifier_key": "uid",
            },
            "schedule": {
                "factory": server.py.services.api.utils.singletons.scheduler.get_scheduler,
                "method_name": server.py.services.api.utils.scheduler.Scheduler.set_schedule_notifications.__name__,
                "identifier_key": "name",
            },
        }
