...
from server.py.services.api.api.endpoints.functions import _build_function
...
class TestNuclioRuntime(TestRuntimeBase):
    ...
    def test_enrich_with_ingress_no_overriding(self, db: Session, client: TestClient):
        ...
        ingresses = server.py.services.api.crud.runtimes.nuclio.helpers.resolve_function_ingresses(
            config["spec"]
        )
    ...
    def test_deploy_python_decode_string_env_var_enrichment(
        ...
        server.py.services.api.utils.runtimes.nuclio.cached_nuclio_version = "1.5.13"
        with pytest.raises(
        ...
        self._reset_mock()
        server.py.services.api.utils.runtimes.nuclio.cached_nuclio_version = "1.8.5"
        function = self._generate_runtime(self.runtime_kind)
        self.execute_function(function)
        self._assert_deploy_called_basic_config(
            expected_class=self.class_name,
        ...
        self._reset_mock()
        server.py.services.api.utils.runtimes.nuclio.cached_nuclio_version = "1.7.5"
        function = self._generate_runtime(self.runtime_kind)
    ...
    def test_is_nuclio_version_in_range(self):
        server.py.services.api.utils.runtimes.nuclio.cached_nuclio_version = "1.7.2"

        assert not server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
            "1.6.11", "1.7.2"
        )
        assert not server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
            "1.7.0", "1.3.1"
        ...
        assert server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
            "1.7.2", "1.7.3"
        )
        assert server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
            "1.7.0", "1.7.3"
        ...
        server.py.services.api.utils.runtimes.nuclio.cached_nuclio_version = ""
        assert server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
            "1.5.5", "2.3.4"
        )
        assert server.py.services.api.crud.runtimes.nuclio.helpers.is_nuclio_version_in_range(
