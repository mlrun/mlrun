...
for auth_method in AUTH_METHODS_AND_REQUIRED_PARAMS:
    generated_pytest_parameters.append((auth_method, False))
    if auth_method.startswith("fsspec"):
        generated_pytest_parameters.append((auth_method, True))
...
class TestAzureBlob:
    def setup_class(self):
        self.profile_name = "azure_blob_ds_profile"

    def get_blob_container_path(self, use_datastore_profile):
        if use_datastore_profile:
            return f"ds://{self.profile_name}/{config['env'].get('AZURE_CONTAINER')}"
        return "az://" + config["env"].get("AZURE_CONTAINER")
    ...
    def test_read_df_dir(
        ...
        with tempfile.NamedTemporaryFile(
            ...
            blob_container_path = self.get_blob_container_path(use_datastore_profile)
            dir_url = f"{blob_container_path}/{blob_dir}/{directory}/{uuid.uuid4()}"
