...
def read_artifact(project, tag, key):
    iter = int(request.args.get('iter', '0'))
    data = app.db.read_artifact(key, tag=tag, iter=iter, project=project)
    return data
...
def list_artifacts():
    ...
    artifacts = app.db.list_artifacts(name, project, tag, labels)
...
def del_artifacts():
    ...
    app.db.del_artifacts(name, project, tag, labels)
    return jsonify(ok=True)
