...
class TestModelMonitoringAPI(TestMLRunSystem):
    ...
    v3io_api = os.environ.get("V3IO_API")
    ...
    def test_clear_endpoint(self, client: TestClient):
        ...
        response = requests.delete(
            url=f"{self.v3io_api}/api/projects/{kv_record.metadata.project}/model-endpoints/{endpoint.metadata.uid}",
    ...
    def test_store_endpoint_update_existing(self):
        ...
        response = requests.put(
            url=f"{self.v3io_api}/api/projects/{endpoint.metadata.project}/model-endpoints/{endpoint.metadata.uid}",
            headers={"X-V3io-Session-Key": auth_info.data_session},
            json=endpoint_dict,
        )
    ...
    def test_list_endpoints(self):
        ...
        response = requests.get(
            url=f"{self.v3io_api}/api/projects/{self.project_name}/model-endpoints",
            headers={"X-V3io-Session-Key": self._get_auth_info().data_session},
        )
    ...
    def test_list_endpoints_filter(self):
        ...
        for i in range(5):
            ...
            mlrun.api.crud.ModelEndpoints().write_endpoint_to_kv(
                self._get_auth_info().data_session, endpoint_details
            )

        filter_model = requests.get(
            f"{self.v3io_api}/api/projects/{self.project_name}/model-endpoints/?model=filterme",
            headers={"X-V3io-Session-Key": access_key},
        )
        assert len(filter_model.json()["endpoints"]) == 1

        filter_labels = requests.get(
            f"{self.v3io_api}/api/projects/{self.project_name}/model-endpoints/?label=filtermex=1",
            headers={"X-V3io-Session-Key": access_key},
        )
        assert len(filter_labels.json()["endpoints"]) == 4

        filter_labels = requests.get(
            f"{self.v3io_api}/api/projects/{self.project_name}/model-endpoints/?label=filtermex=1&label=filtermey=2",
            headers={"X-V3io-Session-Key": access_key},
        )
        assert len(filter_labels.json()["endpoints"]) == 4

        filter_labels = requests.get(
            f"{self.v3io_api}/api/projects/{self.project_name}/model-endpoints/?label=filtermey=2",
            headers={"X-V3io-Session-Key": access_key},
        )
        assert len(filter_labels.json()["endpoints"]) == 4
    ...
    def test_get_endpoint_metrics(self):
        ...
        for i in range(5):
            ...
            response = requests.get(
                url=f"{self.v3io_api}/api/projects/{self.project_name}/model-endpoints/{endpoint.metadata.uid}?metric=predictions_per_second_count_1s",  # noqa
                headers={"X-V3io-Session-Key": self._get_auth_info().data_session},
            )
