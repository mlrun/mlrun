...
class Scheduler:
        ...
        if (
            ...
            if (
                ...
                auth_info.planes = [
                    ...
                    server.py.services.api.utils.clients.iguazio.SessionPlanes.data,
                ]
            ...
            if auth_info.access_key.startswith(
                ...
                secret = server.py.services.api.crud.Secrets().read_auth_secret(
    ...
    def _store_schedule_secrets(
        ...
        if auth_verifier.AuthVerifier().is_jobs_auth_required():
            ...
            # handler
            secret_key_map = server.py.services.api.crud.Secrets().generate_client_key_map_project_secret_key(
                server.py.services.api.crud.SecretsClientType.schedules
            )
            secrets = {
            ...
            if auth_info.username:
                username_secret_key = server.py.services.api.crud.Secrets().generate_client_project_secret_key(
                    server.py.services.api.crud.SecretsClientType.schedules,
                    name,
    ...
    def _get_schedule_secrets(
        ...
        access_key = server.py.services.api.crud.Secrets().get_project_secret(
            project,
            self._secrets_provider,
            schedule_access_key_secret_key,
            allow_secrets_from_k8s=True,
        ...
        username = None
        if include_username:
            schedule_username_secret_key = server.py.services.api.crud.Secrets().generate_client_project_secret_key(
                server.py.services.api.crud.SecretsClientType.schedules,
                name,
                self._secret_username_subtype,
            )
            username = server.py.services.api.crud.Secrets().get_project_secret(
                project,
                self._secrets_provider,
    ...
    def _enrich_schedule_notifications(
        ...
        if schedule_notifications:
            scheduled_object["task"]["spec"]["notifications"] = [
                ...
                for notification in server.py.services.api.api.utils.validate_and_mask_notification_list(
        ...
        if db_schedule and isinstance(db_schedule.scheduled_object, dict):
            ...
            if notifications:
                for notification in notifications:
                    server.py.services.api.api.utils.delete_notification_params_secret(
    ...
    def _submit_run_wrapper(
        ...
        try:
            ...
            _, _, _, response = server.py.services.api.api.utils.submit_run_sync(
    ...
    def schedule_invokable(
        ...
        """
        active_runs = server.py.services.api.crud.Runs().list_runs(
            db_session,
