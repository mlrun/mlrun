...
import (
	...
	"github.com/mlrun/mlrun-go/pkg/proto/health_v1"
...
type MlrunGRPCServer interface {
	...
	SetServingStatus(status health_v1.HealthCheckResponse_ServingStatus)
...
type AbstractMlrunGRPCServer struct {
	...
	Server         *grpc.Server
	servingStatus  health_v1.HealthCheckResponse_ServingStatus
...
func (s *AbstractMlrunGRPCServer) RegisterRoutes(ctx context.Context) {
	...
	health_v1.RegisterHealthServer(s.Server, s)
}
...
func (s *AbstractMlrunGRPCServer) Check(context.Context, *health_v1.HealthCheckRequest) (*health_v1.HealthCheckResponse, error) {
	return &health_v1.HealthCheckResponse{
		Status: s.servingStatus,
	}, nil
}

func (s *AbstractMlrunGRPCServer) Watch(request *health_v1.HealthCheckRequest, stream health_v1.Health_WatchServer) error {
	currentStatus := s.servingStatus

	// send once the status
	if err := stream.Send(&health_v1.HealthCheckResponse{
...
func StartServer(server MlrunGRPCServer, port int, logger logger.Logger) error {
	...
	server.SetServingStatus(health_v1.HealthCheckResponse_SERVING)
...
func (s *AbstractMlrunGRPCServer) SetServingStatus(status health_v1.HealthCheckResponse_ServingStatus) {
