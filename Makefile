...
MLRUN_PREBAKED_MODELS_GPU_IMAGE_NAME_TAG := $(MLRUN_CUDA_VERSION)-base-ubuntu20.04
MLRUN_PREBAKED_MODELS_GPU_PY37_IMAGE_NAME_TAG := 11.0-cudnn8-devel-ubuntu18.04
# set the base image to MLRUN_PREBAKED_MODELS_GPU_PY37_IMAGE_NAME_TAGGED when using python 3.7
MLRUN_MODELS_GPU_BASE_IMAGE_TAG := $(shell echo "$(MLRUN_PYTHON_VERSION)" | awk -F. -v PY39_IMAGE_NAME="$(MLRUN_PREBAKED_MODELS_GPU_IMAGE_NAME_TAG)" -v PY37_IMAGE_NAME="$(MLRUN_PREBAKED_MODELS_GPU_PY37_IMAGE_NAME_TAG)" '{if($$2==7) {print PY37_IMAGE_NAME} else {print PY39_IMAGE_NAME}}')
...
models-gpu: update-version-file ## Build models-gpu docker image
	...
	docker build \
		...
		--build-arg MLRUN_BASE_IMAGE_TAG=$(MLRUN_MODELS_GPU_BASE_IMAGE_TAG) \
