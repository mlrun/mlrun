...
class TestFeatureStoreSparkEngine(TestMLRunSystem):
    ...
    def test_mapval_with_partial_mapping(self):
        # we need this test in order to check partial mapping -> only part of the values in field are replaced.
        key = "patient_id"
    ...
    def test_mapval_with_mixed_types(self):
        ...
        with pytest.raises(
            ...
            match="^Mapvalues - mapping that changing column type must change all values accordingly the column!"
            " Column- hr_is_error$",
        ):
            fstore.ingest(
                measurements,
