...
class TestProjects(TestDatabaseBase):
    ...
    def test_store_project_emits_owner_set_when_changed(self, monkeypatch):
        services.api.crud.Projects().create_project(
            self._db_session,
            self._generate_project_for_event_test("p-owner-change", owner="alice"),
        )
        client = self._mock_events_client(monkeypatch)
        ...
        services.api.crud.Projects().store_project(
            self._db_session,
            "p-owner-change",
            self._generate_project_for_event_test("p-owner-change", owner="bob"),
            auth_info=mlrun.common.schemas.AuthInfo(username="admin"),
        )
        client.generate_project_owner_set_event.assert_called_once_with(
            project_name="p-owner-change",
            prev_owner="alice",
            new_owner="bob",
            actor="admin",
        )
    ...
    def test_store_project_no_event_when_owner_unchanged(self, monkeypatch):
        services.api.crud.Projects().create_project(
            self._db_session,
            self._generate_project_for_event_test("p-same-owner", owner="alice"),
        )
        client = self._mock_events_client(monkeypatch)
        ...
        client.generate_project_owner_set_event.assert_not_called()

    def test_store_project_no_owner_event_when_creating_via_upsert(self, monkeypatch):
        # store_project performs an upsert; when the project does not pre-exist
        # there is no prior owner to compare against, so no Owner.Set event.
        client = self._mock_events_client(monkeypatch)
        services.api.crud.Projects().store_project(
            self._db_session,
            "p-new-via-store",
            self._generate_project_for_event_test("p-new-via-store", owner="alice"),
            auth_info=mlrun.common.schemas.AuthInfo(username="alice"),
        )
        client.generate_project_owner_set_event.assert_not_called()

    def test_patch_project_emits_owner_set_when_patch_contains_new_owner(
        self, monkeypatch
    ):
        services.api.crud.Projects().create_project(
            self._db_session,
            self._generate_project_for_event_test("p-patch-owner", owner="alice"),
        )
        client = self._mock_events_client(monkeypatch)
        ...
        client.generate_project_owner_set_event.assert_called_once_with(
            project_name="p-patch-owner",
            prev_owner="alice",
            new_owner="bob",
            actor="admin",
        )

    def test_patch_project_no_event_when_patch_skips_owner(self, monkeypatch):
        services.api.crud.Projects().create_project(
            self._db_session,
            self._generate_project_for_event_test("p-patch-no-owner", owner="alice"),
        )
        client = self._mock_events_client(monkeypatch)
        ...
        services.api.crud.Projects().patch_project(
            self._db_session,
            "p-patch-no-owner",
            {"spec": {"description": "just a description change"}},
            auth_info=mlrun.common.schemas.AuthInfo(username="admin"),
        )
        client.generate_project_owner_set_event.assert_not_called()

    def test_patch_project_no_event_when_owner_unchanged(self, monkeypatch):
        services.api.crud.Projects().create_project(
            self._db_session,
            self._generate_project_for_event_test("p-patch-same-owner", owner="alice"),
        )
        client = self._mock_events_client(monkeypatch)
