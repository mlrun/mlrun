...
class ServerSideLauncher(launcher.BaseLauncher):
    ...
    def enrich_runtime(
        ...
        server.py.services.api.api.utils.mask_function_sensitive_data(
    ...
    def _enrich_full_spec(
        ...
        server.py.services.api.api.utils.try_perform_auto_mount(
            runtime, self._auth_info
        )

        # Validate function's service-account, based on allowed SAs for the project,
        # if existing in a project-secret.
        server.py.services.api.api.utils.process_function_service_account(runtime)

        server.py.services.api.api.utils.ensure_function_security_context(
            runtime, self._auth_info
        )
    ...
    def _validate_state_thresholds(
        ...
        for state, threshold in state_thresholds.items():
            ...
            try:
                server.py.services.api.utils.helpers.time_string_to_seconds(threshold)
