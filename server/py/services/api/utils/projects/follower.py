...
import server.py.services.api.utils.projects.remotes.leader
import server.py.services.api.utils.projects.remotes.nop_leader
...
class Member(
    ...
    def initialize(self):
        ...
        self._leader_client: server.py.services.api.utils.projects.remotes.leader.Member
        if self._leader_name == "iguazio":
            self._leader_client = server.py.services.api.utils.clients.iguazio.Client()
            ...
            self._leader_client = (
                server.py.services.api.utils.projects.remotes.nop_leader.Member()
            )
        else:
    ...
    def create_project(
        ...
        if server.py.services.api.utils.helpers.is_request_from_leader(
            ...
            server.py.services.api.crud.Projects().create_project(db_session, project)
    ...
    def get_project(
        ...
        return server.py.services.api.crud.Projects().get_project(db_session, name)
    ...
    def list_projects(
        ...
        if (
            ...
            and not server.py.services.api.utils.helpers.is_request_from_leader(
                projects_role, leader_name=self._leader_name
            )
        ):
        ...
        projects_output = server.py.services.api.crud.Projects().list_projects(
    ...
    async def list_project_summaries(
        ...
        return await server.py.services.api.crud.Projects().list_project_summaries(
    ...
    async def get_project_summary(
        ...
        return await server.py.services.api.crud.Projects().get_project_summary(
            db_session, name
        )

    @server.py.services.api.utils.helpers.ensure_running_on_chief
    ...
    def _sync_projects(self, full_sync=False):
        ...
        try:
            ...
            db_projects = server.py.services.api.crud.Projects().list_projects(
                db_session
            )
    ...
    def _project_deletion_background_task_exists(project_name):
        background_task_kinds = [
            ...
            for task_format in [
                server.py.services.api.utils.background_tasks.BackgroundTaskKinds.project_deletion_wrapper,
                server.py.services.api.utils.background_tasks.BackgroundTaskKinds.project_deletion,
            ]
        ]
        return any(
            [
                server.py.services.api.utils.background_tasks.InternalBackgroundTasksHandler().get_active_background_task_by_kind(
