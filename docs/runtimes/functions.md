...
In the vast majority of cases, an MLRun function would require access to storage. This storage
may be used to provide inputs to the function such as data-sets to process or data-streams that contain input events.
Storage would usually also be used to store function outputs and result artifacts, such as trained models or processed
data-sets.

As MLRun function may be distributed and executed in Kubernetes pods, the storage used would in most cases be a shared
storage, and execution pods would need some configurations applied to them to allow the function code access to the 
storage in question. These configurations may be k8s volume mounts, specific environment variables that contain
configuration and credentials and so on. Note that these storage configurations are not applicable to functions running
locally in the development environment, as they are executed in the local context.
...
environment variables need to be configured in the pod, providing the v3io API URL and access keys
...
containers as specific paths in the pod itself, such as `/User`. To enable this, several specific volume mount 
...
4. Others - As use-cases evolve, other cases of storage access may be needed, requiring various configurations to be
applied to function execution pods

MLRun attempts to offload this storage configuration task from the user by automatically applying the most common 
storage configuration to functions. Due to this, in most cases no additional storage configuration will be required 
before executing a function as a Kubernetes pod. The configurations applied by MLRun are:
* In an Iguazio system, apply configurations needed to access `v3io` through the API
* In an open-source deployment where NFS is configured, apply `pvc` configuration to access the NFS storage
...
adding environment variables, mounts and so on. MLRun also provides a set of common modifiers which can be used to apply 
storage configurations.
These modifiers can be applied by using the `.apply()` method on the function, providing the modifier to apply. Some 
examples of using this are found later in this page.

When a different storage configuration is manually applied to a function, MLRun's auto-mount logic is disabled, to 
prevent conflicts between configurations. The auto-mount logic can also be disabled by setting
`func.spec.disable_auto_mount = True` 
on any MLRun function. 

### Modifying auto-mount default configuration
The default auto-mount behavior applied by MLRun can be controlled by setting MLRun configuration parameters. 
For example, the logic can be set to automatically mount `v3io` Fuse driver on all functions, or perform `pvc` mount for
NFS storage on all functions.
For example, the following code can be used to apply `v3io` Fuse driver by default:
