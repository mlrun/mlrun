...
|ML-12078|When Model-monitoring is enabled with V3io configured as default artifact storage, each model-endpoint creates 25 directories per day. These directories consume the MDS heap - and can consume it entirely (on 128GB data-nodes) in ~30 days. |Run a daily cron job to delete parquet partition directories older than a week.|v1.11.0|
