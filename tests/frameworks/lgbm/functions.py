...
def get_model(algorithm_functionality: AlgorithmFunctionality, n_classes: int = N_CLASSES) -> lgb.LGBMModel:
    if algorithm_functionality.is_classification():
        ...
        if algorithm_functionality.is_binary_classification():
            return lgb.LGBMClassifier(params={"objective": "multiclass", "num_class": 2})
        return lgb.LGBMClassifier(params={"objective": "multiclass", "num_class": n_classes})
    return lgb.LGBMRegressor()


def get_params(algorithm_functionality: AlgorithmFunctionality, n_classes: int = N_CLASSES) -> dict:
    if algorithm_functionality.is_classification():
        if algorithm_functionality.is_single_output():
...
class LightGBMHandlers:
    @staticmethod
    def training_api_train(context: mlrun.MLClientCtx, algorithm_functionality: str, model_name: str = None):
        ...
        validation_set_1 = lgb.Dataset(x_test[: len(x_test) // 2], y_test[: len(x_test) // 2])
        validation_set_2 = lgb.Dataset(x_test[len(x_test) // 2 :], y_test[len(x_test) // 2 :])
    ...
    @staticmethod
    def training_api_evaluate(context: mlrun.MLClientCtx, algorithm_functionality: str, model_path: str):
        # TODO: Finish handler once the evaluation is implemented.
        pass

    @staticmethod
    def sklearn_api_train(context: mlrun.MLClientCtx, algorithm_functionality: str, model_name: str = None):
        algorithm_functionality = AlgorithmFunctionality(algorithm_functionality)
    ...
    @staticmethod
    def sklearn_api_evaluate(context: mlrun.MLClientCtx, algorithm_functionality: str, model_path: str):
        algorithm_functionality = AlgorithmFunctionality(algorithm_functionality)
        x, y = get_dataset(algorithm_functionality=algorithm_functionality, for_training=False)
