...
You can use the `additional_filters` attribute while reading from a [ParquetTarget](feature-store/sources-targets.md#parquet-target), similar to `additional_filters` in [ParquetSource](sources-targets.md#parquet-source).
## Training
