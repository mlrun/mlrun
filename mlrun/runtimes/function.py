...
from ..error import error_to_string
...
class RemoteRuntime(KubeResource):
    ...
    def invoke(
        ...
        try:
            ...
            raise OSError(
                f"error: cannot run function at url {path}, {error_to_string(err)}"
            )
        if not resp.ok:
    ...
    def _run(self, runobj: RunObject, execution):
        ...
        try:
            ...
            logger.error(f"error invoking function: {error_to_string(err)}")
