...
def _fix_datasets_large_previews(
    ...
    for artifact in artifacts:
        try:
            ...
            if artifact_dict and artifact_dict.get('kind') == mlrun.artifacts.DatasetArtifact.kind:
                header = artifact_dict.get('header', [])
                if header and len(header) > mlrun.artifacts.dataset.max_preview_columns:
                    logger.debug("Found dataset artifact with more than allowed columns in preview fields. Fixing", artifact=artifact_dict)
                    columns_to_remove = header[mlrun.artifacts.dataset.max_preview_columns:]
                    ...
                    if artifact_dict.get('preview'):
                        ...
                        for preview_row in artifact_dict['preview']:
                            # sanity
                            if len(preview_row) < mlrun.artifacts.dataset.max_preview_columns:
                                logger.warning("Found artifact with more than allowed columns in header definition, "
                                               "but preview data is valid. Leaving preview as is", artifact=artifact_dict)
                            new_preview.append(preview_row[:mlrun.artifacts.dataset.max_preview_columns])
                    ...
                    artifact_dict['header'] = header[:mlrun.artifacts.dataset.max_preview_columns]
                    logger.debug("Fixed dataset artifact preview fields. Storing", artifact=artifact_dict)
                    db._store_artifact(db_session, artifact.key, artifact_dict, artifact.uid, project=artifact.project, tag_artifact=False)
