...
MLRUN_DEV_IMAGE_NAME_TAGGED := $(MLRUN_IMAGE_NAME):$(MLRUN_DOCKER_TAG)$(MLRUN_PYTHON_VERSION_SUFFIX)-dev
...
mlrun: update-version-file ## Build mlrun docker image
	...
	@# build dev version
	docker build \
		--file dockerfiles/mlrun/Dockerfile.dev \
		--build-arg MLRUN_MLRUN_IMAGE=$(MLRUN_IMAGE_NAME_TAGGED) \
		--tag $(MLRUN_DEV_IMAGE_NAME_TAGGED) \
		.
...
push-mlrun: mlrun ## Push mlrun docker image
	...
	docker push $(MLRUN_DEV_IMAGE_NAME_TAGGED)

.PHONY: pull-mlrun
pull-mlrun: ## Pull mlrun docker image
	docker pull $(MLRUN_IMAGE_NAME_TAGGED)
	docker pull $(MLRUN_DEV_IMAGE_NAME_TAGGED)
