...
from server.py.services.api.common.runtime_handlers import get_resource_labels
...
class DaskRuntimeHandler(BaseRuntimeHandler):
    ...
    def _enrich_list_resources_response(
        ...
        if not enrich_needed:
            return response
        services = server.py.services.api.utils.singletons.k8s.get_k8s_helper().v1api.list_namespaced_service(
        ...
        for service in services.items:
            try:
                if force or service.metadata.name in service_names:
                    server.py.services.api.utils.singletons.k8s.get_k8s_helper().v1api.delete_namespaced_service(
