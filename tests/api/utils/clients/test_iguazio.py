...
from server.py.services.api.utils.asyncio import maybe_coroutine
...
async def test_verify_request_session_success(
    api_url: str,
    iguazio_client: server.py.services.api.utils.clients.iguazio.Client,
    requests_mock: requests_mock_package.Mocker,
    aioresponses_mock: aioresponses_mock,
):
...
async def test_create_project_failures(
    ...
    _mock_job_progress(
        ...
        session,
        job_id,
        server.py.services.api.utils.clients.iguazio.JobStates.failed,
        job_result,
    )
...
async def test_delete_project_job_is_done(
    ...
    responses = [
        functools.partial(
            _mock_get_job,
            server.py.services.api.utils.clients.iguazio.JobStates.completed,
            "",
            session,
        ),
    ]
...
def _mock_job_progress(
    ...
    session: str,
    job_id: str,
    terminal_job_state: str = server.py.services.api.utils.clients.iguazio.JobStates.completed,
    job_result: str = "",
):
    def _mock_get_job(state, result, session, request, context):
        context.status_code = http.HTTPStatus.OK.value
    ...
    responses = [
        functools.partial(
            _mock_get_job,
            server.py.services.api.utils.clients.iguazio.JobStates.in_progress,
            job_result,
            session,
        ),
        functools.partial(
